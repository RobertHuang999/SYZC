...
> **字段基准**：[`GPS设备字段清单.md`](./GPS设备字段清单.md) 是本功能所有字段、枚举和字段规则的唯一基准
...
列表字段定义继承自[`GPS设备字段清单.md`](./GPS设备字段清单.md)。列表业务字段展示顺序如下：

> 序号、设备原名称、系统内名称、设备编码、车辆信息、绑定仓库、具体位置、修改人员、更新时间。

序号固定为列表第一列，仅为当前页展示序号，不作为设备主键。设备状态仅作为筛选字段，不参与列表展示。GPS设备当前不定义“绑定状态”字段，也不提供“绑定状态”筛选项；设备是否绑定仅通过“绑定仓库”字段展示和筛选。

“操作”属于列表交互列，不作为业务数据字段。当前行操作包括重命名、绑定仓库、实时定位、运动轨迹和移除设备；页面级按钮“GPS平台规则配置”和“GPS平台基础数据”不属于行操作。

### 4.2 字段取值规则

| 字段 | 规则 |
| :--- | :--- |
| 设备原名称 | 三方平台返回设备名称；首次同步时作为系统内名称默认值 |
| 系统内名称 | 首次默认取设备原名称；用户重命名保存后，以人工维护值为准，后续三方名称变化不得覆盖 |
| 设备编码 | 设备唯一识别编码；编码格式、长度和生成方待研发确认；所有设备维护请求使用设备编码作为幂等关联键 |
| 车辆信息 | 当前资料明确展示三方平台返回的车牌号；车辆信息结构、车牌号格式和变更规则待确认 |
| 设备状态 | 当前定位状态；原型当前编码为1离线、2停车、3行驶、4静止，编码和判定规则待确认；仅作为列表筛选项 |
| 绑定仓库 | 使用仓库唯一标识保存绑定关系，列表展示仓库名称；无绑定时为空 |
| 具体位置 | 展示设备在绑定仓库中的安装或布设位置；无值时展示“未设置” |
| 修改人员 | 由服务端根据认证身份写入最近一次更新设备信息的操作人 |
| 更新时间 | 由服务端写入最近一次更新设备信息的时间，格式为 `YYYY-MM-DD HH:mm:ss`；当前不参与筛选 |
...
| 设备绑定情况 | 可见规则 |
| :--- | :--- |
| 已绑定仓库 | 登录用户拥有对应仓库数据权限时可见；无对应仓库权限时不返回设备数据 |
| 未绑定仓库 | 当前租户内具备有效账号且拥有 GPS设备菜单查看权限的用户可见 |
...
| 页面项目 | 展示规则 | 规则说明 |
| :--- | :--- | :--- |
| 车牌号 | 当前车辆车牌号码，例如“辽JGJ440” | 引用“车辆信息”中的车牌号 |
| 状态 | 当前车辆状态及持续时长，例如“停车1天”“离线5天” | 状态枚举、状态判定来源和状态更新时间待确认 |
| 司机姓名 | 车辆对应司机姓名 | 司机关联关系和无值展示规则待确认 |
| 手机号码 | 司机手机号码 | 按角色执行敏感信息脱敏，脱敏规则待确认 |
| 当前位置 | 当前定位地址，精确到街道门牌号 | 地址解析失败或无地址时的展示规则待确认 |
| 更新时间 | 定位信息最后更新时间，格式为 `YYYY-MM-DD HH:mm:ss` | 以定位数据服务端时间为准，不等同于设备基础信息的更新时间 |
| 系统名称 | 定位设备名称，例如“定位设备02#” | 对应设备字段“系统内名称”；原型说明中的“系统内容”建议统一为“系统名称” |
| 设备序号 | 设备在系统中的序列名称，例如“LU005” | 是否对应独立持久化字段待确认 |
| 设备编号 | 设备唯一标识码，例如“4534534534534” | 对应设备字段“设备编码” |
...
| 展示区域 | 展示规则 |
| :--- | :--- |
| 历史轨迹地图 | 展示历史轨迹地图和车辆行驶路径；地图底图服务及交互能力待确认 |
| 状态分段列表 | 按车辆状态分段展示，一个状态对应一段轨迹记录 |
| 状态分段卡片 | 展示更新时间、系统内名称、设备状态、司机姓名、车牌号、状态持续时长、起止时间，并提供“查看详情”入口 |
| 轨迹详情 | 点击“查看详情”后展示该段轨迹的车辆、司机、起止地址、总里程和定位设备信息 |
| 位置展示 | 状态分段列表当前不展示位置；轨迹详情中展示结束地址作为历史轨迹终点地址 |
| 性能约束 | 地址解析过多可能导致页面卡顿；地址解析范围、缓存和性能阈值待确认 |
...
| :--- | :--- | :--- |
| 车牌号 | 当前车辆车牌号码，例如“辽JGJ440” | 引用“车辆信息”中的车牌号 |
| 状态持续时长 | 当前设备状态持续时长 | 展示格式沿用实时定位页面规则；无值时原型展示“--” |
| 司机姓名 | 车辆对应司机姓名 | 无值展示规则待确认 |
| 手机号 | 司机手机号码 | 按角色执行敏感信息脱敏 |
| 开始地址 | 历史轨迹起点地址 | 地址解析规则和无地址展示规则待确认 |
| 结束地址 | 历史轨迹终点地址 | 状态分段列表不展示，详情中展示 |
| 总里程 | 本次轨迹总行驶距离，单位为 `km` | 里程计算规则和异常值处理待确认 |
| 更新时间 | 定位信息最后更新时间，格式为 `YYYY-MM-DD HH:mm:ss` | 以定位数据服务端时间为准 |
| 系统内名称 | 定位设备名称，例如“定位设备02#” | 对应设备字段“系统内名称” |
| 设备原名称 | 设备出厂或原始名称，例如“LU005” | 与三方平台原始设备名称的映射关系待确认 |
| 设备编号 | 设备唯一标识码，例如“4534534534534” | 对应设备字段“设备编码” |
| 地图展示 | 历史轨迹地图和车辆行驶路径 | 属于页面展示区域，不作为独立持久化业务字段 |
...
1. 所有列表查询和操作接口必须同时校验菜单功能权限、操作权限、租户权限、机构权限和绑定仓库数据权限。
2. 已绑定仓库设备按仓库数据权限控制；未绑定仓库设备按当前租户、账号有效性和 GPS设备菜单查看权限控制。
...
| 编号 | 验收项 |
| :--- | :--- |
| A01 | 列表字段顺序为序号、设备原名称、系统内名称、设备编码、车辆信息、绑定仓库、具体位置、修改人员、更新时间 |
...
| 编号 | 待补充内容 | 影响范围 |
| :--- | :--- | :--- |
| 1 | 设备编码生成方、编码格式、长度和接口字段编码 | GPS设备基础字段、维护接口和幂等关联 |
| 2 | 车辆信息结构、车牌号格式、车辆与司机关联关系及变更规则 | 车辆信息、实时定位和轨迹详情 |
| 3 | 实时定位状态枚举、状态判定来源、自动刷新频率和手动刷新规则 | 实时定位页面和状态展示 |
| 4 | 设备序号是否为独立持久化字段及其生成规则 | 实时定位弹窗和轨迹详情 |
| 5 | 地图底图服务、坐标系、地址解析服务、失败兜底和性能阈值 | 实时定位和运动轨迹地图 |
| 6 | 历史轨迹接口字段、状态分段算法、里程计算、轨迹点清洗和地址解析规则 | 运动轨迹查询和详情 |
| 7 | GPS平台联登协议、回调方式、平台地址、会话有效期和异常提示文案 | GPS平台规则配置、GPS平台基础数据 |
| 8 | 手机号码、司机信息和定位信息的角色脱敏规则 | 实时定位、运动轨迹和审计日志 |

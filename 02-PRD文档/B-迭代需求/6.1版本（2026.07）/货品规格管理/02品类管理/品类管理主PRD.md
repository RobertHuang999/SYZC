...
| :--- | :--- |
| - 品类列表查询与所属大类/名称/状态多维筛选；<br/>- 新建品类（支持动态定义最多 5 个规格参数维度）；<br/>- 编辑品类与规格（入库引用强拦截）；<br/>- 品类启用（强校验所属大类启用状态）；<br/>- 品类停用（事务内级联停用下属货品并保存快照）；<br/>- 品类删除（阶段一启用拦截 + 阶段二下属货品入库单扫描）；<br/>- 操作留痕与全路径审计追溯。 | - 具体货品（SKU）规格值与计量单位维护（由 03货品模块承载）；<br/>- 入库收集信息配置与单价维护；<br/>- 入库、出库、加工与质押业务办理；<br/>- 跨大类批量品类迁移。 |
...
```mermaid
flowchart LR
    A[01 大类管理<br/>顶层分类] -->|1:N 映射| B[02 品类管理<br/>规格参数维度定义]
    B -->|1:N 细分| C[03 货品管理<br/>SKU 主档与入库采集]

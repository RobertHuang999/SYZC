...
> | **核心** | [门禁开锁审批配置字段清单](./门禁开锁审批配置字段清单.md) | 字段定义、筛选/列表/新增编辑/详情、展示顺序与控件规格 |
> | **核心** | [门禁开锁审批配置业务规则规格](./门禁开锁审批配置业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
> | *衍生* | ASCII 线框 / Demo 文档 | 由三件套派生的可视化与原型输入，**非独立真源** |
>
> **版本**：V1.6 | **更新时间**：2026-09-08 | **责任人**：产品
>
...
开锁审批配置模块为管理人员提供集中维护开锁审批策略的能力：支持按**指定设备**灵活定义审批节点（指定人员含启用/停用账号，指定角色为启用角色）、审批超时时间，并实现策略的版本化留痕与启停用管理。
...
2. **租户内灵活选人**：审批节点支持指定人员（**启用/停用均可**）与指定角色，支持跨机构协同审批与风控穿透。
...
| 上游依赖 | **门禁设备主数据**：适用设备；**机构与组织架构**：本租户内机构树；**人员账号 / RBAC 角色**：指定人员（本租户内启用/停用账号均可）、指定角色（本租户内启用角色） |
...
| 模块 | 职责 |
| :--- | :--- |
| **开锁审批配置（本模块）** | 创建、编辑、启停用审批规则；维护配置版本；指定人员含启用/停用账号，指定角色为启用角色 |
...
> 本节描述**做什么、怎么交互**；字段/控件见《[字段清单](./门禁开锁审批配置字段清单.md)》；匹配与版本规则见《[业务规则规格](./门禁开锁审批配置业务规则规格.md)》。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 筛选、查看开锁审批配置，进入新增/编辑/详情 |
| 菜单路径 | `配置管理 → 业务流程管理 → 开锁审批` |
| 页面路径 | `/配置管理/业务流程管理/开锁审批` |
| 骨架结构 | 页头 + 筛选 Card + 数据表格 + 分页 |

**筛选区**：配置名称（模糊）、状态（全部/已启用/已停用）；展开行配置编号（精确）。Header 集成「+ 新增配置」（P02）。
...
编辑已启用配置 → 保存生成新版本（旧版已停用）；已停用不可编辑；**仅已停用**可逻辑删除（C08）。
...
**表单要点**：配置名称、适用设备（多选挂锁/人脸门禁）、审批节点（指定人员/角色，本租户 R02）、审批超时时间。**审批方式**系统默认「任一人通过」，前端不展示。

| 对比项 | 新增 | 编辑 |
| :--- | :--- | :--- |
| 配置名称/适用设备 | 可填 | **Readonly** |
| 保存后状态 | 已启用，Version=1 | 修改节点/超时 → 新版本已启用 |

**禁止项**：无草稿态；保存即已启用。

### 4.3 PC 详情页

只读展示配置编号、Version、审批节点、适用设备、停用原因（如有）；已停用展示删除入口。
...
| 场景ID | 场景 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| S01 | 新增指定设备配置并立即生效 | **主流程** | 填配置名称、勾选适用设备、从**本租户内**选择审批节点人员/角色、设审批超时时间 → 保存 → 状态=已启用，配置版本=1；审批方式系统默认为任一人通过 |
...
| S04 | 审批节点混配跨机构人员与角色 | **支线** | 节点1=指定角色「监管主管」，节点2=指定本租户内其他机构人员「张三」→ 审批方式=任一人通过 → 各节点解析出的 eligible 人员并行待办，任一人通过即完成 |
| S05 | 设备冲突阻断 | **异常** | 同一设备已存在于内容不一致的已启用配置 → 新增保存阻断，提示与配置{编号}存在设备冲突（R03） |
| S06 | 审批节点无效阻断 | **异常** | 指定人员不存在/跨租户，或指定角色无启用成员/跨租户 → 保存阻断，提示审批节点{序号}无效（R02） |
...
| R02 | 审批节点中：指定人员须为**本租户内**存在的账号（**启用/停用均可**）；指定角色须为**本租户内**启用的角色且至少 1 个启用成员 |
...
### 8.4 审批节点解析与可见性规则（供下游申请侧使用）

| 规则ID | 规则 |
| :--- | :--- |
| R28 | 指定角色节点在申请提交时展开为当时角色下启用账号并写入快照；在途不重算 |
| R30 | 申请提交时角色节点展开后 eligible 人员为空（含排除申请人后、或账号/角色已失效）则阻断——**校验执行在申请侧，本模块保证配置保存时 R02 有效** |
...
| P01b | **审批对象候选范围约束**：**指定人员**为本租户内全部账号（**含启用与停用**）；**指定角色**为本租户内启用角色 | 下拉按租户边界过滤；指定人员不按账号状态过滤 |
...
> 适用设备选择器的候选范围遵循门禁设备模块既有权限；审批节点人员与角色选择器候选范围遵循 P01b 本租户边界。
...
| 指定人员账号不存在或不属于当前租户 | R02 阻断，提示「保存/启用失败：审批节点{序号}无效或不属于当前租户」 |
| 指定角色无启用成员或不属于当前租户 | R02 阻断 |
...
| :--- | :--- | :--- | :--- | :--- |
| V01 | 新增指定设备配置成功 | 有新增权限；设备主数据有效 | 填写配置名称、勾选适用设备、从本租户内选择至少1个审批节点、设审批超时时间 → 保存 | 生成配置编号；状态=已启用；配置版本=1；审批方式=任一人通过 |
| V02 | 新增后直接已启用 | 同 V01 | 新增保存 | 表单无状态、无审批方式选项；保存后列表状态=已启用 |
| V03 | 配置名称租户内唯一 | 已存在同名配置 | 新增同名配置保存 | 阻断；提示名称重复 |
| V04 | 审批节点指定人员 | 本租户内有人员账号（启用或停用） | 节点选指定人员（展示「姓名（所属机构）」） | 保存成功；详情展示节点序号、审批对象类型、指定人员及机构 |
| V04a | 指定停用人员 | 本租户内有停用人员账号 | 节点选指定停用人员 | 保存成功；不因账号停用阻断 |
| V05 | 审批节点指定角色 | 本租户内角色有启用成员 | 节点选指定角色 | 保存成功；详情展示指定角色 |
| V06 | 无效审批节点阻断 | 指定人员跨租户/不存在，或角色无启用成员 | 保存 | 阻断；Toast「保存/启用失败：审批节点{序号}无效或不属于当前租户」 |
| V06a | 租户隔离候选限制 | 存在跨租户账号/角色 | 查看审批节点人员/角色下拉候选项 | 下拉列表仅出现本租户内的人员与角色，跨租户数据不可见（P01b） |
...
| 2026-09-08 | V1.5 | 审批对象候选范围调整为**本租户（系统租户）内**全员可选；P01b、R02、验收 V06a 及阻断文案同步更新 |
| 2026-09-08 | V1.6 | **指定人员**放开账号启用/停用限制；补充验收 V04a |
| 2026-09-11 | V1.7 | **对齐三件套标杆**：补 §四 页面功能说明；章节重编号 |

...
3. 历史配置中的通知渠道、对象、升级天数/对象迁入对应策略子项；`notify_channels` 迁移时须剔除「站内信」（6.2 改为 H5「押品预警信息」待处置红点自动展示，不可配置）及误写入的「Webhook」（6.2 外部渠道仅保留短信/邮件）；6.1 抵/质押率（LTV）历史配置必须拆成补仓线、平仓线并映射为 6.2 `LTV_DUAL`（ORD-R10 整卡单等级）。
4. IoT 五类继续区分 `LEGACY_IOT` 与 `IOT_PENETRATION`，不得伪装成 06 商业策略；有设备事件关联的历史记录保留 `related_device_event_id`。
5. 6.2 运行时 `熔断状态` 统一写 `NONE`。旧熔断值仅写 `legacy_熔断状态` 审计字段，不得在迁移时锁定出库或解押。
...
`collateral_warn_ledger` 必须至少具备：`warn_id`、租户/订单快照、`rule_id`、`strategy_type`、`severity_level_id`、`severity_code`、`severity_name`、`severity_color`、`sort_order_snapshot`、`预警来源`、`legacy_warn_type`、`related_device_event_id`、`trigger_snapshot`、`process_status`、`熔断状态`、`legacy_熔断状态`、`publicity_status`、`event_source`、`migration_batch_id`。
...
| 6.1 源类型 | 6.2 处理 | 目标 `strategy_type` / `预警来源` |
| :--- | :--- | :--- |
...
| 监管业务率异常 | 保留并拆双阈值 | 6.1 源类型 `抵/质押率（LTV）` → 6.2 `strategy_type=LTV_DUAL` / `ORDER_CONFIG` |
...
把旧表转换到临时表 `stg_order_warn_config`：统一租户、订单类型、源类型、规则名称、原始等级、参数 JSON、通知 JSON 和来源配置 ID。以下情况写 `migration_exception`，不直接写目标有效数据：租户/订单不存在、源类型无法识别、同一订单类型冲突、等级多匹配、LTV_DUAL 双阈值缺一、监管服务订单带 `LOAN_RISK` 贷中策略（贷中仍仅抵押/质押适用）。
...
- 商业六类：预警来源为「订单预警配置」，通过历史规则 ID 或订单+策略类型回填综合规则与子项；等级取触发时原始等级映射，不取当前规则等级覆盖历史。
- 有 `related_device_event_id` 的五类 IoT：`warn_type=IOT_PENETRATION`、预警来源为「现场物联穿透告警」。
- 无设备事件关联的五类 IoT：`warn_type=LEGACY_IOT`、`预警来源=LEGACY_IOT`，保留 `legacy_warn_type`，只读展示。
- 所有历史流水 `event_source=HISTORY_MIGRATION`、`migration_batch_id=:batch_id`；`熔断状态业务熔断状态（无熔断）`，原值写 `legacy_熔断状态`。
- 已处理/未处理/无效状态按旧处置状态一一映射；不得因规则删除或历史熔断值改变处置状态。

### Step 5：批次收尾与启用

1. 校验每条 `ACTIVE` 主规则至少有一条 策略处于开启状态 子项。
2. 校验监管服务订单没有 `LOAN_RISK` 子项（贷中策略仍仅抵押/质押）；`LTV_DUAL` 允许监管服务订单保留。
3. 校验每个启用子项等级 ID 均属于对应租户；`LTV_DUAL` 补仓/平仓共用单等级（ORD-R10），且 `close_out.sort_order >= margin_call.sort_order`。
...
| 顺序 | 步骤 | 责任 | 通过条件 |
| :---: | :--- | :--- | :--- |
| **【03/01 等级与租户映射】(1)** | 03/01 等级与租户映射 | 配置管理员/数据管理员 | 每个租户映射完整，ID 可解析 |
| **【05 IoT 事件关联准备】(2)** | 05 IoT 事件关联准备 | IoT 服务 | 事件 ID 可对账，无法关联的进入历史标签 |
| **【06 源数据清洗与异常分流】(3)** | 06 源数据清洗与异常分流 | 订单服务/数据管理员 | 无未解释的源类型、等级、订单冲突 |
| **【主表+子项迁移】(4)** | 主表+子项迁移 | 订单服务 | 一订单一综合规则，策略子项唯一 |
| **【押品流水迁移】(5)** | 押品流水迁移 | 风险信息服务 | 历史快照保留，运行时熔断全为 `NONE` |
| **【对账、抽样、回放】(6)** | 对账、抽样、回放 | 测试/风控 | 通过第五章全部校验和跨域用例 |
| **【切换与观察】(7)** | 切换与观察 | 发布负责人 | 批次完成，异常清单闭环 |

## 修订记录

| 日期 | 版本 | 变更 |
| :--- | :---: | :--- |
| 2026-08-21 | **V2.0** | 重做迁移方案：按订单聚合综合规则、拆策略子项、动态等级 ID 映射、抵/质押率（LTV, Loan-to-Value） 双等级、批次化回滚、异常隔离与熔断审计 |

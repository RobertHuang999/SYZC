...
3. 历史配置中的通知渠道、对象、升级天数/对象迁入对应策略子项；`notify_channels` 迁移时须剔除「站内信」（6.2 改为 H5「押品预警信息」待处置红点自动展示，不可配置）及误写入的「Webhook」（6.2 外部渠道仅保留短信/邮件）；抵/质押率历史配置必须拆成补仓线、平仓线及各自等级。
...
| 抵/质押率异常 | 保留并拆双阈值 | `抵/质押率（LTV, Loan-to-Value）` / `ORDER_CONFIG` |
...
把旧表转换到临时表 `stg_order_warn_config`：统一租户、订单类型、源类型、规则名称、原始等级、参数 JSON、通知 JSON 和来源配置 ID。以下情况写 `migration_exception`，不直接写目标有效数据：租户/订单不存在、源类型无法识别、同一订单类型冲突、等级多匹配、抵/质押率（LTV, Loan-to-Value） 双阈值缺一、监管订单带 抵/质押率（LTV, Loan-to-Value）/贷中策略。
...
2. 校验监管订单没有 `抵/质押率（LTV, Loan-to-Value）`/`LOAN_RISK` 子项。
3. 校验每个启用子项等级 ID 均属于对应租户；抵/质押率（LTV, Loan-to-Value） 两个等级均存在且 `close_out.sort_order >= margin_call.sort_order`。

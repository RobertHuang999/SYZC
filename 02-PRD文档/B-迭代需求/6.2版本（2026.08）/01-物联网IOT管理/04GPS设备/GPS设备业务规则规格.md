...
> **文档定位**：**三件套核心**——状态机、动作约束、校验规则、计算联动、权限与跨模块流程的**唯一定义来源**。配套：[主PRD](./GPS设备主PRD.md)（业务与页面功能）、[字段清单](./GPS设备字段清单.md)（字段）。ASCII/Demo 为衍生，只引用本文件规则 ID，不重复定义规则本体。
...
| 规则编号 | 规则 |
| :--- | :--- |
| **【设备类型与范围】(R01)** | 实时定位和运动轨迹按当前用户、租户、机构和仓库数据权限查询，服务端不得只依赖前端筛选。 |
| **【业务标识唯一性】(R02)** | 运动轨迹查询开始时间至结束时间不得超过31天；超出范围阻止请求。 |
| **【名称继承与保护】(R03)** | 实时定位展示当前车辆、司机、位置、状态和更新时间；司机手机号按角色脱敏。 |
| **【数据权限强隔离】(R04)** | 轨迹详情展示起止地址、总里程、状态持续时长和定位设备信息；未确认接口字段不得固化为数据库字段。 |

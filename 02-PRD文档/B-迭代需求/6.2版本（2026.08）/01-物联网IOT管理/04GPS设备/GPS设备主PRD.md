...
> | **核心** | [GPS设备字段清单](./GPS设备字段清单.md) | 字段定义、列表/行操作/操作字段清单、展示顺序与控件规格 |
> | **核心** | [GPS设备业务规则规格](./GPS设备业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
...
| 上游依赖 | **三方 GPS 平台**提供设备 ID、定位坐标、在线状态和轨迹数据；**仓库档案**提供仓库/库房/分区 ID及设备空间归属；**账号与数据权限**提供租户、机构和仓库可见范围；**车辆及司机数据**提供设备与车辆/司机的关联展示。来源：[GPS设备字段清单](./GPS设备字段清单.md)、REF-WH、REF-ACCOUNT、REF-RBAC；车辆/司机字段来源当前 B 目录未定义，按待确认项处理。 |
| 下游引用 | 仓储监管、车辆监控、风险识别、订单追溯、设备审计及其他定位业务 |
| 业务单据 | 不生成独立业务单据，不建立单据状态机 |

## 4. 页面功能说明

> 本节描述**做什么、怎么交互**；列表/筛选字段见《[GPS设备字段清单](./GPS设备字段清单.md)》；行操作字段见《[GPS设备操作字段清单索引](./02-操作字段清单/操作字段清单索引.md)》。操作规则细节见 §5 及业务规则规格。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 管理车载 GPS 设备，查看实时定位与历史运动轨迹 |
| 页面路径 | `/物联网IOT管理/GPS设备` |
| 骨架结构 | 页头 + 筛选区 + 数据表格（含操作列）+ 分页 |

**筛选区**：设备名称、车辆信息、设备状态、仓库、具体位置等；控件与匹配方式见字段清单「筛选区」。

**行操作**（按设备类型/权限显隐）：重命名、绑定仓库、实时定位、运动轨迹、移除设备。各操作弹窗、校验与联动见 §5。

**地图子页**：实时定位与运动轨迹为行操作打开的地图容器，数据来自三方 GPS 平台（见 §5.4/§5.5）。
...
| :--- | :--- | :---: | :---: | :--- |
| 重命名 | 列表“操作”列 | 是 | 否 | 继承[监控设备主 PRD·5.2 重命名](../01监控设备/监控设备主PRD.md#52-重命名) |
| 绑定仓库 | 列表“操作”列 | 是 | 是，仓库权限和有效订单校验 | 继承[监控设备主 PRD·5.3 仓库绑定](../01监控设备/监控设备主PRD.md#53-仓库绑定) |
| 实时定位 | 列表“操作”列 | 否 | 是，三方 GPS 定位数据和地图服务 | 本文档第5.4节 |
| 运动轨迹 | 列表“操作”列 | 否 | 是，三方 GPS 历史轨迹和地图服务 | 本文档第5.5节 |
| 移除设备 | 列表“操作”列 | 是 | 是，订单、规则、通知和仓库绑定 | 继承[监控设备主 PRD·5.7 移除设备](../01监控设备/监控设备主PRD.md#57-移除设备) |

重命名、绑定仓库和移除设备使用与监控设备一致的权限、校验、并发控制、幂等、跨模块联动、异常处理和审计规则。GPS设备不复制或另行改写上述继承规则；监控设备规则发生变更时，本模块同步继承最新版本。

### 5.2 重命名

重命名继承[监控设备主 PRD·5.2 重命名](../01监控设备/监控设备主PRD.md#52-重命名)，本模块执行以下约束：
...
仓库绑定继承[监控设备主 PRD·5.3 仓库绑定](../01监控设备/监控设备主PRD.md#53-仓库绑定)。GPS设备本期不新增独立绑定模型，沿用监控设备的仓库、库房、分区层级校验和有效订单阻断规则。
...
实时定位弹窗的设备上下文、车辆信息、定位返回、地图标记和页面状态字段以[实时定位字段清单](./04-操作字段清单/03实时定位字段清单.md)为准；本主 PRD 仅定义权限、查询时机、地图交互和异常处理。
...
地图、状态分段卡片、轨迹详情和位置展示字段以[运动轨迹字段清单](./04-操作字段清单/04运动轨迹字段清单.md)为准。业务规则为：按车辆状态分段展示轨迹，状态分段提供详情入口；地址解析应避免一次性解析过多点，缓存和性能阈值由技术方案确认。

#### 5.5.3 轨迹详情字段

运动轨迹查询条件、状态分段、轨迹详情、地图展示和第三方返回字段以[运动轨迹字段清单](./04-操作字段清单/04运动轨迹字段清单.md)为准；本主 PRD 仅定义查询范围、权限、流程和异常处理。
...
移除设备继承[监控设备主 PRD·5.7 移除设备](../01监控设备/监控设备主PRD.md#57-移除设备)。GPS设备执行移除时，沿用监控设备的确认弹窗、说明必填、订单/规则/通知/仓库解绑、失败回滚、历史快照保留和定时重新加入规则。
...
| 系统内名称 | 物联网IOT管理配置/系统带出/用户维护 | 首次继承设备原名称；人工重命名后保存当前值，三方名称变化不得覆盖 |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **【GPS设备列表与筛选】(P-01)** | GPS设备列表与筛选 | 设备管理员、监管人员 | 设备、车辆、状态、仓库筛选 | 查看列表与筛选 | 按仓库和机构数据权限返回 |
| **【设备名称和仓库维护】(P-02)** | 设备名称和仓库维护 | 设备管理员 | 重命名、绑定仓库、位置维护 | 重命名、仓库绑定 | 继承设备通用并发和审计规则 |
| **【实时定位】(P-03)** | 实时定位 | 监管、风控 | 地图、车辆标记、信息弹窗 | 实时定位 | 定位设备、车辆和司机信息脱敏展示 |
| **【运动轨迹】(P-04)** | 运动轨迹 | 监管、审计 | 时间范围查询、轨迹地图、分段详情 | 历史轨迹 | 查询范围不超过31天 |
| **【GPS平台外部入口】(P-05)** | GPS平台外部入口 | 配置管理员 | 规则配置、基础数据联登 | 未纳入本次移动端范围 | 入口权限独立于设备列表权限 |
| **【移除设备】(P-06)** | 移除设备 | 设备管理员 | 移除确认和历史快照 | 移除设备 | 失败保留原状态和关联 |
| **【操作审计】(P-07)** | 操作审计 | 审计、管理员 | 查询维护、定位和跳转记录 | 未纳入本次移动端范围 | 记录请求上下文和第三方结果 |
...
- 行操作字段见[操作字段清单索引](./02-操作字段清单/操作字段清单索引.md)。

## 11. 验收重点

| 编号 | 验收项 |
| :--- | :--- |
| **【列表字段及顺序以《GPS】(A01)** | 列表字段及顺序以《GPS设备字段清单》为准 |
| **【“设备名称”仅查询系统内】(A02)** | “设备名称”仅查询系统内名称；设备原名称变化不得覆盖人工维护的系统内名称 |
| **【GPS设备不定义绑定状态】(A03)** | GPS设备不定义绑定状态字段，也不提供绑定状态筛选项；仓库筛选使用绑定仓库唯一标识 |
| **【更新时间在字段清单中标记】(A04)** | 更新时间在字段清单中标记为不可筛选，车辆信息和具体位置支持模糊匹配，设备状态支持下拉精确筛选 |
| **【已绑定位置设备按所属仓库】(A05)** | 已绑定位置设备按所属仓库数据权限可见；未绑定位置设备按租户、账号有效性和菜单权限可见 |
| **【行操作仅包含重命名、绑定】(A06)** | 行操作仅包含重命名、绑定仓库、实时定位、运动轨迹和移除设备 |
| **【重命名、绑定仓库和移除设】(A07)** | 重命名、绑定仓库和移除设备完整继承监控设备对应规则，不产生独立分叉逻辑 |
| **【实时定位展示地图、车辆位】(A08)** | 实时定位展示地图、车辆位置、状态、司机、车牌、当前位置、更新时间和定位设备信息 |
| **【实时定位无权限或设备编码】(A09)** | 实时定位无权限或设备编码不匹配时不得返回或展示其他设备定位数据 |
| **【运动轨迹仅展示当前用户有】(A10)** | 运动轨迹仅展示当前用户有仓库权限的数据，查询范围不超过31天 |
| **【运动轨迹按状态分段展示，】(A11)** | 运动轨迹按状态分段展示，支持查看详情；状态持续时长按天、小时、分钟换算 |
| **【状态持续时长无值时展示“】(A12)** | 状态持续时长无值时展示“--”；当天数和小时数均为0时不展示0天和0小时 |
| **【地址解析失败不阻断轨迹主】(A13)** | 地址解析失败不阻断轨迹主线展示；状态分段列表不展示位置，轨迹详情展示起止地址 |
| **【GPS平台规则配置和GP】(A14)** | GPS平台规则配置和GPS平台基础数据分别由独立配置权限控制展示，并通过联登跳转三方平台 |
| **【三方定位、轨迹、地图和联】(A15)** | 三方定位、轨迹、地图和联登失败时有明确提示，不产生错误的设备基础数据变更 |
| **【查询、重命名、绑定、解绑】(A16)** | 查询、重命名、绑定、解绑、移除、定位、轨迹和联登失败均记录审计日志 |
| **【所有维护请求使用设备编码】(A17)** | 所有维护请求使用设备编码或设备主键，不使用列表序号作为请求关联条件 |
| **【设备移除成功后有效列表不】(A18)** | 设备移除成功后有效列表不展示，历史轨迹和审计数据按快照规则保留 |

## 12. 三方接口待确认事项

| 编号 | 待确认内容 | 责任人/角色 | 确认时点 | 验收依据 |
| :--- | :--- | :--- | :--- | :--- |
| **【设备编码生成方、编码格式】(1)** | 设备编码生成方、编码格式、长度和接口字段编码 | GPS平台对接研发 | 接口评审前 | 编码样例、唯一性和幂等关联规则明确 |
| **【车辆信息结构、车牌号格式】(2)** | 车辆信息结构、车牌号格式、车辆与司机关联及手机号字段 | GPS平台对接研发、车辆数据负责人、安全负责人 | 接口评审前 | 车辆、司机和脱敏样例可供联调验收 |
| **【状态枚举、编码、判定来源】(3)** | 状态枚举、编码、判定来源、优先级、自动刷新频率和手动刷新规则 | GPS平台对接研发、前端负责人 | 原型评审和接口联调前 | 列表、实时定位、轨迹状态和刷新请求行为一致 |
| **【地图底图、坐标系、地址解】(4)** | 地图底图、坐标系、地址解析服务、失败降级和性能阈值 | GPS平台对接研发、地图服务负责人 | 技术方案评审前 | 地图不可用、地址解析失败和大数据量加载有降级方案 |
| **【历史轨迹接口、轨迹点结构】(5)** | 历史轨迹接口、轨迹点结构、状态分段算法和轨迹点清洗规则 | GPS平台对接研发 | 历史轨迹接口联调前 | 31天查询、轨迹点、分段和空数据响应结构明确 |
| **【里程单位、精度、计算责任】(6)** | 里程单位、精度、计算责任方和异常值处理 | GPS平台对接研发、车辆数据负责人 | 历史轨迹接口联调前 | 总里程样例、单位精度及缺失/异常值处理可验收 |
| **【GPS平台联登协议、回调】(7)** | GPS平台联登协议、回调方式、平台地址、会话有效期和异常提示文案 | GPS平台对接研发、安全负责人 | 联登联调前 | 联登成功、权限不足、会话过期、回调失败和平台不可用均有测试用例 |
| **【设备序号是否为独立持久化】(8)** | 设备序号是否为独立持久化字段及其生成规则 | 设备管理研发 | 技术设计评审前 | 明确序号仅为列表展示还是持久化字段 |

...
> | **核心** | [人脸配置字段清单](./人脸配置字段清单.md) | 字段定义、列表/详情/授权配置、展示顺序与控件规格 |
> | **核心** | [人脸配置业务规则规格](./人脸配置业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
...
> 本节描述**做什么、怎么交互**；列表/筛选字段见《[人脸配置字段清单](./人脸配置字段清单.md)》。操作规则细节见 §5 及业务规则规格。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 维护人员人脸/密码凭证并下发至已绑定人脸门禁设备 |
| 页面路径 | `/物联网IOT管理/人脸配置` |
| 骨架结构 | 页头 + 筛选区 + 数据表格（含操作列）+ 分页 |

**筛选区**：姓名、账号、机构等（见字段清单）；控件与匹配方式见字段清单「筛选区」。

**行操作**（按设备类型/权限显隐）：上传人脸、更新人脸、删除人脸、查看门禁数据。各操作弹窗、校验与联动见 §5。

**门禁数据详情**：点击行内门禁数据进入只读详情；敏感字段（照片大图、密码明文）需单独权限并审计。
...
| 人脸照片 | 用户上传 | 私有对象存储（OSS）/私有存储库存储加密文件，保存生成图像唯一索引与访问 Token |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **【人脸配置列表】(F-01)** | 人脸配置列表 | 人脸管理员、审计 | 启用账号筛选、状态查看 | 未纳入本次移动端范围 | 按租户、机构和账号权限过滤 |
| **【上传或替换人脸】(F-02)** | 上传或替换人脸 | 人脸管理员 | 图片上传、替换和大图查看 | 未纳入本次移动端范围 | JPG/JPEG/PNG，≤200KB；查看大图单独授权 |
| **【配置开锁密码】(F-03)** | 配置开锁密码 | 人脸管理员 | 密码录入、唯一性校验和下发 | 未纳入本次移动端范围 | 密码原文不写日志，至少配置人脸或密码一项 |
| **【分配门禁】(F-04)** | 分配门禁 | 人脸管理员 | 单用户选择设备、授权和覆盖下发 | 未纳入本次移动端范围 | 单次最多10台设备，按设备仓库权限过滤 |
| **【批量分配门禁】(F-05)** | 批量分配门禁 | 人脸管理员 | 跨页选择账号和设备批量下发 | 未纳入本次移动端范围 | 最多10个账号，结果按账号和设备拆分 |
| **【查看下发进度】(F-06)** | 查看下发进度 | 人脸管理员、审计 | 进度列表、设备级结果和失败原因 | 未纳入本次移动端范围 | 普通失败即最终失败，由用户重新发起 |
| **【移除门禁授权】(F-07)** | 移除门禁授权 | 人脸管理员 | 门禁数据列表逐条移除 | 未纳入本次移动端范围 | 只解除账号与设备关系，不删除设备主数据 |
| **【账号状态联动】(F-08)** | 账号状态联动 | 系统、管理员 | 停用/删除时撤销凭证和授权 | 不适用 | 撤销失败进入后台失败重试队列 |
...
| 编号 | 验收项 |
| :--- | :--- |
| **【人脸配置列表字段及顺序以】(A01)** | 人脸配置列表字段及顺序以《人脸配置字段清单》为准 |
| **【列表仅展示账号状态为“启】(A02)** | 列表仅展示账号状态为“启用”的人员账号；账号脱敏显示手机号 |
| **【人脸照片（≤200KB）】(A03)** | 人脸照片（≤200KB）与 4~8 位数字开锁密码选填组合，提交时校验必须至少完成一项 |
| **【开锁密码在同一租户范围内】(A04)** | 开锁密码在同一租户范围内校验唯一性，重复时拦截并提示“密码已存在，请重新设置” |
| **【查看开锁密码明文（点击眼】(A05)** | 查看开锁密码明文（点击眼睛图标）及查看人脸大图需单独权限校验并写入审计日志 |
| **【分配门禁及批量分配门禁的】(A06)** | 分配门禁及批量分配门禁的前置显示与勾选条件为“人脸状态为已录入 或 开锁密码为已录入” |
| **【单用户分配门禁单次提交校】(A07)** | 单用户分配门禁单次提交校验最多勾选 10 个门禁设备；未选择设备拦截提示“请至少选择一个设备！” |
| **【批量分配门禁最多勾选 1】(A08)** | 批量分配门禁最多勾选 10 个账号，支持跨页勾选；已分配门禁允许覆盖，下发成功后替换原关联 |
| **【普通用户主动分配或更新凭】(A09)** | 普通用户主动分配或更新凭证时，下发失败即最终失败：设备离线或下发异常时直接记为失败，不离线等待与上线自动重试；用户需重新发起操作 |
| **【全局“查看进度”打开“进】(A10)** | 全局“查看进度”打开“进度记录”列表，可下钻查看设备级下发状态和失败原因说明 |
| **【“门禁数据”详情仅支持逐】(A11)** | “门禁数据”详情仅支持逐条“移除”关联，不影响设备主数据；物联网IOT管理中门禁设备被移除时同步解绑关联 |
| **【当账号在系统【账号管理】】(A12)** | 当账号在系统【账号管理】中停用或删除时，系统自动触发异步任务向已绑定设备下发删除凭证指令 |
| **【密码原文与人脸照片二进制】(A13)** | 密码原文与人脸照片二进制严禁写入日志文件 |
| **【并发提交或更新时使用版本】(A14)** | 并发提交或更新时使用版本锁控制，阻止错误覆盖；账号停用/删除触发的凭证撤销失败进入后台失败重试队列 |

## 12. 待补充与优化事项

| 编号 | 待补充/优化内容 | 影响范围 | 说明 |
| :--- | :--- | :--- | :--- |
| **【门禁凭证授权时间段与有效期控制】(1)** | 门禁凭证授权时间段与有效期控制 | 门禁鉴权、接口扩展 | 三方接口支持授权有效期，当前版本为长期有效，后期 PRD 迭代记为可优化项 |
| **【人脸状态枚举编码及进度记】(2)** | 人脸状态枚举编码及进度记录查询/保留规则 | 研发接口与页面查询 | 需在技术设计阶段确认枚举、排序、分页和历史记录保留周期；本期不新增任务编号字段 |

...
> | **核心** | [监控设备字段清单](./监控设备字段清单.md) | 字段定义、列表/行操作/操作字段清单、展示顺序与控件规格 |
> | **核心** | [监控设备业务规则规格](./监控设备业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
...
> 本节描述**做什么、怎么交互**；列表/筛选字段见《[监控设备字段清单](./监控设备字段清单.md)》；行操作字段见《[监控设备操作字段清单索引](./02-操作字段清单/操作字段清单索引.md)》。操作规则细节见 §5 及业务规则规格。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 管理视频监控与人脸门禁设备台账，执行绑定、直播、回放、锚点与移除 |
| 页面路径 | `/物联网IOT管理/监控设备` |
| 骨架结构 | 页头 + 筛选区 + 数据表格（含操作列）+ 分页 |

**筛选区**：设备分类、绑定状态、仓库等；控件与匹配方式见字段清单「筛选区」。

**行操作**（按设备类型/权限显隐）：重命名、仓库绑定、查看直播、查看回放、图像锚点、移除设备。各操作弹窗、校验与联动见 §5。

**子页/弹窗**：直播、回放、图像锚点为行操作打开的弹窗或全屏容器，非独立菜单。

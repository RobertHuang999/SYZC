...
> | **核心** | [门禁设备字段清单](./门禁设备字段清单.md) | 字段定义、列表/行操作/操作字段清单、展示顺序与控件规格 |
> | **核心** | [门禁设备业务规则规格](./门禁设备业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
...
> 本节描述**做什么、怎么交互**；列表/筛选字段见《[门禁设备字段清单](./门禁设备字段清单.md)》；行操作字段见《[门禁设备操作字段清单索引](./02-操作字段清单/操作字段清单索引.md)》。操作规则细节见 §5 及业务规则规格。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 管理挂锁与人脸门禁设备，执行绑定、密码获取、设备数据与移除 |
| 页面路径 | `/物联网IOT管理/门禁设备` |
| 骨架结构 | 页头 + 筛选区 + 数据表格（含操作列）+ 分页 |

**筛选区**：设备类型、绑定状态、仓库等；控件与匹配方式见字段清单「筛选区」。

**行操作**（按设备类型/权限显隐）：重命名、仓库绑定、设备数据、获取门锁密码（挂锁）、获取门禁密码（人脸）、人脸配置、移除设备。各操作弹窗、校验与联动见 §5。

**密码双路径**：先匹配审批配置，命中走 07 发起申请，未命中免审直发（R31 人脸不发短信）。规则见 §5.5。
...
| 获取门锁密码 | 仅挂锁门禁 | 列表“操作”列 | 否 | §5.5：先匹配审批；**免审**→取消有效期（仅事由+备注）+短信+引导我的申请记录；**需审批**→07 发起申请（无有效期） |
...
| 设备类型 | 行操作名称 | 免审弹窗核心交互 |
| :--- | :--- | :--- |
| **挂锁门禁** | 【获取门锁密码】 | 事由、备注（**取消有效期**） → 确认后短信下发（“...您的开锁密码为XXXXXX，请凭此密码前往设备开锁...”）；引导至【我的申请记录】查看密码；凭证固定为【已下发】（同设备后续申请执行阻止流程） |
| **人脸门禁** | 【获取门禁密码】 | 事由、**有效期**、开锁次数（1~100）、备注 → 确认后引导至【我的申请记录】查看 6 位临时密码（**不调用短信**） |

需审批分支表单字段与免审**完全一致**（挂锁：事由+备注，取消有效期；人脸：事由+有效期+开锁次数+备注），见 [01发起开锁申请字段清单](../../07-审批中心/03-业务审批/04-我的申请管理/04-开锁审批/02-操作字段清单/01发起开锁申请字段清单.md)。

#### 5.5.3 挂锁门禁【获取门锁密码】规则

1. **入口与权限**：仅挂锁行展示【获取门锁密码】；须具备对应角色权限。
2. **免审**：未命中需审批时，打开获取门锁密码弹窗；字段与流程以 [04获取门锁密码字段清单](./02-操作字段清单/04获取门锁密码字段清单.md) 为准（事由、备注，无有效期）。
3. **需审批**：命中需审批时，**不**进入免审弹窗，切换为 [发起开锁申请](../../07-审批中心/03-业务审批/04-我的申请管理/04-开锁审批/开锁申请主PRD.md)（无有效期）；提交后创建 `UNLOCK_APPLY` 单据，审批通过后由 07 模块触发密码服务 + 短信下发。
4. **业务规则（免审）**：须绑定手机号；确认后向绑定手机号发送短信（文案：“...您的开锁密码为XXXXXX，请凭此密码前往设备开锁...”）；**弹窗内不展示明文**，引导至【我的申请记录】·开锁申请查看；**同时**创建 `approval_required=false` 开锁记录，凭证固定为【已下发】；同设备后续再次申请执行阻止流程。
...
3. **需审批**：命中需审批时，切换为发起开锁申请弹窗（字段与免审一致：挂锁取消有效期仅事由+备注；人脸含有效期+开锁次数）；**不要求绑定手机号**；**任何路径均不调用短信**。
4. **业务规则（免审）**：表单校验通过后生成 6 位数字密码；**弹窗内不展示明文**，引导至【我的申请记录】查看；**同时**创建 `approval_required=false` 开锁记录并写入「获取密码成功」门禁事务。
...
| 2 | **挂锁**门锁动态密码短信模板配置 | 挂锁已取消有效期，短信文案为“...您的开锁密码为XXXXXX，请凭此密码前往设备开锁...”；人脸为 6 位系统唯一数字（**无短信模板**） |
...
| 挂锁门锁密码 | 密码服务 / 短信网关 | 动态生成下发，不存储明文；引导至【我的申请记录】查看（取消有效期填写，凭证固定为已下发；短信文案提示凭此密码前往设备开锁） |
...
| G-04 | 获取门锁密码 | 有对应敏感权限的人员 | **免审**：挂锁事由/备注（取消有效期）+短信+我的申请记录查看；**需审批**：转 07 发起申请（无有效期） | 获取密码（含审批路由） | 先匹配审批配置（§5.5）；取消有效期，固定已下发；同设备后续申请阻止；禁止记录明文 |
...
| A08 | **免审**：挂锁弹出获取门锁密码弹窗（事由+备注，取消有效期）；人脸弹出获取门禁密码（事由+有效期+次数+备注，无短信）。**需审批**：两类设备均打开 07 发起开锁申请弹窗（字段与免审一致） |
...
| 2 | **挂锁**门锁动态密码短信模板配置 | 可优化项；挂锁已取消有效期，文案为“...您的开锁密码为XXXXXX，请凭此密码前往设备开锁...”；人脸为6位数字（**无短信模板**） |

...
| 获取门锁密码 | 仅挂锁门禁 | 列表“操作”列 | 否 | §5.5：先匹配审批；**免审**→有效期表单+短信+引导我的申请记录；**需审批**→07 发起申请 |
...
| 设备类型 | 行操作名称 | 免审弹窗核心交互 |
| :--- | :--- | :--- |
| **挂锁门禁** | 【获取门锁密码】 | 事由、**有效期**、备注 → 确认后短信下发；引导至【我的申请记录】查看密码（有效期最长 24h，超时凭证失效） |
| **人脸门禁** | 【获取门禁密码】 | 事由、**有效期**、开锁次数（1~100）、备注 → 确认后引导至【我的申请记录】查看 6 位临时密码（**不调用短信**） |

需审批分支表单字段与免审**完全一致**（挂锁：事由+有效期+备注；人脸：事由+有效期+开锁次数+备注），见 [01发起开锁申请字段清单](../../07-审批中心/03-业务审批/04-我的申请管理/04-开锁审批/02-操作字段清单/01发起开锁申请字段清单.md)。
...
4. **业务规则（免审）**：须绑定手机号；确认后向绑定手机号发送短信；**弹窗内不展示明文**，引导至【我的申请记录】·开锁申请查看；**同时**创建 `approval_required=false` 开锁记录并写入「获取密码成功」门禁事务。
...
3. **需审批**：命中需审批时，切换为发起开锁申请弹窗（字段与免审一致：挂锁含有效期；人脸含有效期+开锁次数）；**不要求绑定手机号**；**任何路径均不调用短信**。
...
| 2 | **挂锁**门锁动态密码短信模板配置 | 挂锁与人脸均支持用户选择有效期（最长 24 小时）；人脸为 6 位系统唯一数字（**无短信模板**） |
...
| 挂锁门锁密码 | 密码服务 / 短信网关 | 动态生成下发，不存储明文；引导至【我的申请记录】查看（用户选择有效期，最长 24h） |
...
| G-04 | 获取门锁密码 | 有对应敏感权限的人员 | **免审**：挂锁事由/有效期/备注+短信+我的申请记录查看；**需审批**：转 07 发起申请 | 获取密码（含审批路由） | 先匹配审批配置（§5.5）；免审最长24h+短信；禁止记录明文 |
...
| A08 | **免审**：挂锁弹出获取门锁密码弹窗（事由+有效期+备注）；人脸弹出获取门禁密码（事由+有效期+次数+备注，无短信）。**需审批**：两类设备均打开 07 发起开锁申请弹窗（字段与免审一致） |
...
| 2 | **挂锁**门锁动态密码短信模板配置 | 可优化项；挂锁与人脸均最长 24h 用户选择有效期；人脸为6位数字（**无短信模板**） |

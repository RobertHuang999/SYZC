...
- 通知渠道（短信/邮件，选填）、预警对象、升级预警天数与升级对象；预警命中时移动端「设备预警信息」入口自动展示待处置红点，无需配置；仅勾选短信/邮件后才展示预警对象与升级配置；短信/邮件须同时配置通知对象并勾选渠道后才会下发
...
| 预警大类 + 全局新设备 : 规则 | **1:1**（租户内有效规则唯一，见 R05） |
...
| 通知与升级 | 短信/邮件（选填）；仅勾选渠道后展示预警对象与升级配置；移动端「设备预警信息」待处置红点自动展示，无需配置；首次外部通知按所选渠道下发；**升级预警仅通过短信逐级上报**；`RECORD_ONLY` 隐藏升级 |
...
| RUL-07 | 全局新设备监听 | IoT 写权限 | 勾选开关 | — | R05 每大类限 1 条有效规则（含停用） |
...
| S10 | 全局新设备唯一（含停用） | **异常** | 图像识别大类已有 **停用** 的全局新设备规则 → 再新建同大类「设备上线 + 仅针对新设备」→ R05 阻断 → 提示编辑或删除后重建 |
...
| R05 | 全局新设备规则：每预警大类仅 1 条有效规则（含生效中、停用） |
...
| V14 | 全局新设备唯一（含停用） | 同大类已有停用状态全局新设备规则，再新建同组合 | 保存阻断，提示「该类型全局新设备规则已存在，请编辑或删除后重建」 |
...
| 2026-09-16 | **V4.0** | **R05 收紧**：全局新设备规则创建时即校验唯一性（含停用）；补 S10/V14 场景与验收 |
| 2026-09-11 | **V3.8** | **对齐三件套标杆**：补 §4 页面功能说明；三件套/衍生文头；章节重编号 |
| 2026-09-16 | **V4.0** | **H5 待处置红点口径**：通知渠道说明由「系统小角标」调整为 H5「设备预警信息」待处置红点自动展示；外部通知须配置对象并勾选渠道 | §2 功能范围、§4.2 通知与升级 |
| 2026-09-16 | **V4.0** | **H5 待处置红点口径**：通知渠道说明改为 H5「设备预警信息」红点自动展示；外部通知须配置对象并勾选渠道 | §2 功能范围、§4.2 通知与升级 |
| 2026-09-18 | **V4.1** | **移动端待处置红点口径**：对外表述由 H5 统一为「移动端」；通知与升级区仅勾选短信/邮件后才展示预警对象与升级配置 | §2 功能范围、§4.2 通知与升级 |
| 2026-09-18 | **V4.2** | **升级预警渠道口径**：升级督办通知仅通过短信逐级上报；表单 Checkbox 文案同步 | §4.2 通知与升级 |

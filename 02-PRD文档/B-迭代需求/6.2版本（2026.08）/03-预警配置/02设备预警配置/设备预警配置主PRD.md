...
- **处置策略**（`disposition_mode`）：三档单选——触发即结案 / 人工解除结案 / 恢复自动结案；子类型 chip 系统推荐默认（R15）；安防/图像/挂锁破坏/密码错误禁止恢复自动结案（R15c）；决定 02/01 流水落账方式（R15b）
...
| R15 | 处置策略三档必填；系统推荐 + 用户可配；`RECORD_ONLY` 禁配升级（R15a）；落账联动 02/01（R15b）；安防/图像类禁止 `AUTO_RECOVER`（R15c） |
| R06 | 编辑时名称/类型/子类型不可改 |
...
| V06 | 删除流水联动 | 规则有 3 条未处理预警，执行删除 | 3 条置「已作废」，升级终止 |

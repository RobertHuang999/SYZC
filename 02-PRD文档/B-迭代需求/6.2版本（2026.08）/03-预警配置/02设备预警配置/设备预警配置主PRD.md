...
> **规则来源**：状态机、校验、联动与权限详见《[设备预警配置业务规则规格](./设备预警配置业务规则规格.md)》——**规则 ID 以该文件为 单一真实数据源 (SSOT)，本文仅摘要引用**
> **字段基准**：《[设备预警配置字段清单](./设备预警配置字段清单.md)》为字段唯一定义来源
> **菜单路径**：`预警配置 → 设备预警配置`
> **页面线框**：[列表页](./设备预警配置_ASCII_列表页.md) · [详情页](./设备预警配置_ASCII_详情页.md) · [新增/编辑页](./设备预警配置_ASCII_新增编辑页.md)
> **权限基准**：[6.2全局契约与RBAC权限矩阵](../../6.2全局契约与RBAC权限矩阵.md) 第三章第2节
...
- 通知渠道（站内信/短信/邮件）、预警对象、升级预警天数与升级对象
...
| RUL-02 | 新增规则 | IoT 写权限 | 独立页面表单 | — | R01/R02 防抖；R03~R05 唯一性 |
...
| S03 | 全局新设备上线通知 | **主流程** | 选监控设备上线+勾选仅新设备 → 隐藏设备选择与升级预警（R13）→ 仅配通知渠道与预警对象 → 后续新设备入库即通知 |
...
| V04 | 全局新设备 | 选设备上线+勾选仅新设备 | 设备选择隐藏；后续新设备入库触发通知 |
...
| 2026-08-20 | **V3.0** | **6.2 标杆重构**：对齐业务单据 PRD 章节（1~10）；排除范围 (Out of Scope)；业务场景 S01~S08；权限/边界/验收 V01~V12；状态机与规则下沉至业务规则规格 单一真实数据源 (SSOT)；删重复 第4章 与已合并业务流程引用 |

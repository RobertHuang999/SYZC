...
> **版本**：V3.4 | 2026-09-10（订单货物行与可选标识字段对齐原型）
...
- 订单选择与联动加载（按订单数据权限；监管订单自动禁用抵/质押率、贷中风控卡片）；订单货物由主数据自动带出并全部平铺，超时策略不提供手工添加批次或删除货物入口
...
| :---: | :--- | :--- | :--- | :--- | :--- |
| 1 | **解抵/质押/监管超时** | 超时天数（订单货物二维码/批次行自动带出；只填写超时天数） | 抵/质押、监管 | 订单货物对应超时天数到期且未解押 | 完成解押/出库 |

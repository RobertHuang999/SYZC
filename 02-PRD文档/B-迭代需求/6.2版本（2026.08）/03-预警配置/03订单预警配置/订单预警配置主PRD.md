...
> **版本**：V3.1 | 2026-08-21（升级为一站式多策略综合规则模式）
> **读者**：研发工程师、测试工程师、风控产品经理、架构师
> **规则来源**：状态机、校验、联动与权限详见《[订单预警配置业务规则规格](./订单预警配置业务规则规格.md)》——**规则 ID 以该文件为 单一真实数据源 (SSOT)，本文仅摘要引用**
> **字段基准**：《[订单预警配置字段清单](./订单预警配置字段清单.md)》为字段唯一定义来源
> **菜单路径**：`预警配置 → 订单预警配置`
> **页面线框**：[列表页](./订单预警配置_ASCII_列表页.md) · [详情页](./订单预警配置_ASCII_详情页.md) · [新增/编辑页](./订单预警配置_ASCII_新增编辑页.md)
> **权限基准**：[6.2全局契约与RBAC权限矩阵](../../6.2全局契约与RBAC权限矩阵.md) 第三章第3节
...
- 风控人员可对单个订单“一次性配齐”全部风控监控项，消除重复建单的繁琐操作；
...
- 6 类风控子策略的一站式卡片式表单配置（超时、跌价、盘点、巡检、抵质押率、贷中风控）
...
- 各子项独立通知渠道（站内信/短信/邮件/Webhook）、预警对象、升级预警天数与升级对象
- 规则新增、编辑、软删除、详情查看
...
| ORD-C02 | 新增综合预警规则 | 风控写权限 | 卡片式一站式表单 | — | R04 订单唯一性；R07a 至少启1项；R12 订单类型限制 |
| ORD-C03 | 编辑综合规则 | 风控写权限 | 卡片式一站式表单 | — | R06 不可变字段；R07 乐观锁；C01a 子项关闭联动 |
...
| 场景ID | 场景 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| S01 | 新增抵质押订单全套风控策略 | **主流程** | 选抵/质押订单 → 一次性勾选开启跌价(10%/L3)、质押率(75%/85%)、巡检(王五/7天)等项 → 各自配置通知人 → 一键保存并生效 |
| S02 | 新增监管订单风控策略 | **主流程** | 选监管订单 → 抵/质押率与贷中风控卡片自动置灰禁用 → 开启跌价、巡检与超时项 → 保存生效 |
| S03 | 局部调整风控参数 | **支线** | 编辑已有规则 → 修改跌价比例为 15% 或添加新巡检人 → 保存 → 后续命中按新参数，不影响未变动子项 |
| S04 | 独立关闭某项风控子策略 | **支线** | 编辑规则关闭「价格下跌」开关 → 保存 → 停止跌价监控，历史跌价未处理流水置无效（C01a），其余子项正常 |
| S05 | 开启贷中风控并同步台账 | **主流程** | 开启贷中风控卡片 → 选择智风控模型 → 保存 → C04 幂等同步至贷中管理台账 |
...
| # | 验收项 | 输入条件 | 预期结果 |
| :--- | :--- | :--- | :--- |
| V01 | 一站式多项配置 | 选抵质押订单，同时开启跌价、质押率、巡检 | 一次性保存成功，列表正确展示已启用 3 项徽章 |
| V02 | 监管订单禁用质押率/贷中 | 选择监管订单 | 抵/质押率异常与贷中风控卡片置灰不可选（R12） |
| V03 | 重复配置订单 | 选择已配置规则的订单提交新增 | 保存阻断，提示订单已配置规则（R04） |
| V04 | 至少启用一项校验 | 全部 6 项卡片均关闭点击保存 | 保存阻断，提示请至少启用一项预警策略（R07a） |
| V05 | 独立通知精准触达 | 跌价配业务员，平仓线配风控总监 | 跌价触发仅通知业务员，平仓触发仅通知风控总监 |
| V06 | 局部关闭子项联动 | 编辑规则关闭「价格下跌」 | 仅价格下跌未处理流水置无效，巡检流水保留（C01a） |
| V07 | 整单删除联动 | 删除整条规则 | 关联的所有未处理押品预警置无效，停止升级（C01/C02） |
| V08 | 贷中风控台账同步 | 开启贷中风控卡片保存 | 贷中风控管理台账幂等创建/更新（C04） |
| V09 | 编辑不可变字段 | 编辑已有规则 | 规则名称与关联订单置灰不可改（R06） |
| V10 | 乐观锁并发校验 | 双用户同时编辑先后提交 | 后提交者 R07 乐观锁失败提示刷新 |

...
在供应链金融大宗商品存货质押与监管服务业务中，货权质押期长、市场价格波动大、仓储作业频繁，资金方与监管方需要对抵押、质押及监管服务订单进行全生命周期的风险监控。
...
> **当前订单类型口径（2026-09-15）**：本模块统一使用三类订单类型：`抵押`、`质押`、`监管服务`。三类类型均可按策略适用范围新增、编辑、保存并触发订单预警；旧历史 `监管` 类型已下线，不属于当前枚举，不在筛选、列表、详情或新预警链路展示，也不得映射为 `监管服务`。
...
- 各启用的风控子项独立绑定预警等级：读取当前租户 03/01 预警等级中状态为“启用”的等级，租户等级总数限制为 2~20 档；展示编码、名称和颜色，提交 `severity_level_id`，服务端校验租户归属、启用状态和等级 ID，严重度比较使用 `sort_order`。抵/质押率异常虽配置补仓线、平仓线双阈值，但整个子项仅绑定一个预警等级（REF-SEVERITY，R10）。
- 订单选择与联动加载（按订单数据权限；新增候选返回有效抵押、质押、监管服务订单）；订单货物由主数据自动带出并全部平铺，超时策略不提供手工添加批次或删除货物入口
- 各子项独立通知渠道（短信/邮件，选填）、预警对象、升级预警天数与升级对象；预警命中时移动端「押品预警信息」入口自动展示待处置红点，无需配置；仅勾选短信/邮件后才展示预警对象与升级配置；首次外部通知按所选渠道下发；**升级预警仅通过短信逐级上报**；短信/邮件须同时配置通知对象并勾选渠道后才会下发
- 规则新增、编辑、软删除、详情查看（订单类型为抵押、质押、监管服务；策略按能力矩阵适用）
...
| 配置来源 | 用户手动新增/编辑抵押、质押、监管服务订单规则（6.2 当前唯一写入方式）；旧历史 `监管` 类型不再写入 |
| 上游依赖 | **03/01 预警等级**：提供启用等级下拉、`severity_level_id` 和 `sort_order`；**订单主数据**：提供抵押、质押、监管服务三类订单的 ID、生命周期状态和货权关系；**盘点/巡检任务**：提供任务状态、完成时间和责任人；**外部市价/估值**：提供价格和货值计算输入；**智风控平台**：提供贷中模型结果；**RBAC 与订单数据权限**：过滤列表、订单选择器和写接口，并执行服务端二次校验。旧历史 `监管` 类型不作为当前订单主数据返回。来源：[跨文档引用口径与来源索引](../../../README.md) REF-SEVERITY/REF-ORDER/REF-RBAC/REF-NOTIFY。 |
...
| :---: | :--- | :--- | :--- | :--- | :--- |
| 1 | **解抵押/解质押/监管服务超时** | 超时天数（订单货物二维码/批次行自动带出；只填写超时天数） | **三类订单** | 抵押/质押订单未完成解押，或监管服务订单超过服务期限未结案 | 解押/出库，或监管服务结案 |
| 2 | **价格下跌** | 下跌比例（%） | **仅抵/质押** | 货值跌幅严格大于配置比例 | 估值回升或补仓 |
| 3 | **盘点异常** | 开关勾选即启用 | **三类订单** | 盘点账实不一致 | 盘点差异审批/复核 |
| 4 | **巡检异常** | 巡检人 + 独立周期（天） | **三类订单** | 各责任人超期未巡检 | 提交有效巡检记录 |
| 5 | **抵/质押率异常** | 补仓线/平仓线双阈值 + 分线解除方式 + 单等级 | **仅抵/质押** | LTV 严格大于对应阈值；双线同时命中取平仓线（R13d） | 按命中线展示对应解除方式（R13e） |
| 6 | **贷中风控预警** | 智风控模型（单选） | **仅抵/质押** | 模型异步拒绝/高风险 | 人工复核或单据解除 |

> 订单类型只允许 `抵押`、`质押`、`监管服务` 三项；旧历史 `监管` 不展示、不兼容、不触发新预警。
...
**筛选区**：规则名称、订单号、订单类型、已启用预警项（6 类多选）、状态。订单类型筛选仅提供「全部 / 抵押 / 质押 / 监管服务」；Header 集成「+ 新增订单规则」。

**数据表格**：

- 列：规则名称、订单号、订单类型、货主/货物摘要、已启用预警项徽章集合、状态、更新时间、操作
- 已启用项摘要示例：`[跌价(L3)] [质押率(L3)]`
...
| 骨架结构 | 页头 → 基本信息卡片 → **6 大策略卡片组**（Switch + 折叠 + 逐条保存） |

**页头**：仅「返回/取消」；**无整页统一保存**（V3.2 起各策略独立「保存该策略」）。

**基本信息**：规则名称、关联订单（编辑 Readonly R06）；新增可选择当前租户有权限且状态有效的抵押、质押或监管服务订单，选订单后自动带出订单类型、货主、货物（`cargoItems[]` 逐项平铺，R02）。旧历史 `监管` 类型不返回，旧地址不转换为 `监管服务`。

**6 大策略卡片**（每张含 Switch、专属参数、独立等级、通知/升级、**「保存该策略」**）：

| # | 策略 | 适用说明 | 要点 |
| :---: | :--- | :---: | :--- |
| 1 | 价格下跌 | 仅抵押/质押 | 跌幅比例 + 等级 + 通知 |
| 2 | 抵/质押率双控 | 仅抵押/质押 | 当前 LTV 只读 + 补仓/平仓双阈值 + 分线解除方式 + 单等级 |
| 3 | 仓储巡检超期 | 三类订单 | 巡检人 + 周期动态表格 |
| 4 | 解抵押/解质押/监管服务超时 | 三类订单，按订单类型生成对应超时类型 | 订单货物自动生成行；仅填写超时天数 |
| 5 | 盘点异常 | 三类订单 | 盘点差异配置 |
| 6 | 贷中风控 | 仅抵押/质押 | 模型参数与台账同步 |

**抵/质押率卡片交互（策略 2）**：

- 只读展示**当前订单抵/质押率 (LTV)**（如 `88.50%`）；未录入放款或货值评估未完成时展示 `—`，并在其下方紧邻说明原因（对齐抵质押单据 DZY-R08a）。
- 有当前率时展示刻度示意条（当前点 / 补仓线 / 平仓线），辅助配置阈值。
- **双行表格**（列：类型 / 触发条件 / 解除方式）：触发条件文案为「若 抵/质押率 (LTV) 超过 [X]% 时触发预警（不含等于该阈值）」；解除方式为多选 checkbox（各线独立配置）。
- 卡片底部短提示说明命中与解除规则（R13d/R13e）。
- 整个卡片仅 1 个预警等级下拉（与其他策略一致）。
- **命中判定（R13d）**：仅命中补仓线 → 认定为补仓线；同时命中补仓线与平仓线 → 认定为平仓线。
- **解除展示（R13e）**：02/02 押品预警解除跳转至订单后，仅展示该条预警命中线配置的解除方式。
...
| 能力 | 卡片式只读展示各策略快照、Version、审计；当前抵押、质押、监管服务规则按状态提供编辑/删除 |
| 巡检超期展示 | 策略 3 以表格只读展示「序号 / 巡检人 / 巡检周期」多行配置（如「孙巡检(仓管部) · 每 7 天超期预警」），与新增/编辑页动态表格结构一致 |
...
| 场景ID | 场景 | 类型 | 说明 |
| :--- | :--- | :--- | :--- |
| S01 | 新增抵质押订单全套风控策略 | **主流程** | 选抵/质押订单 → 逐条开启并保存跌价(10%/L3)、质押率(补仓75%/平仓85%/分线解除/L3)、巡检(王五/7天)等策略 → 各子项分别生效 |
| S02 | 查询已失效订单规则 | **只读流程** | 从列表进入已失效规则 → 查看详情、审计和资料；不支持编辑，按状态矩阵处理删除 |
| S03 | 局部调整风控参数（阈值重算置换） | **支线** | 编辑已有规则 → 修改跌价比例（如 10% 改为 20%）、超时天数或质押率阈值 → 点击该卡片「保存该策略」→ 系统立即重算当前订单：若仍符合新阈值且存在未解除预警，旧流水置「已作废」（C07）并生成新预警推送；若不符合新阈值则重新进行新一轮判断；未变动子项不受影响 |
...
> 旧历史 `监管` 类型不进入当前数据集；`监管服务` 是独立的新业务类型，不承担旧类型兼容展示职责。
...
| R12 | **订单类型枚举**：当前只允许 `抵押`、`质押`、`监管服务`；旧历史 `监管` 不进入当前订单选择器、筛选、列表、详情和新预警触发链路，不得映射为 `监管服务` |

#### 8.2 各子项参数与独立等级

| 规则ID | 规则摘要 |
| :--- | :--- |
| R08~R09 | 抵质押率：补仓线/平仓线双阈值必填，平仓线严格高于补仓线 |
...
| R20a/R24a | **修改升级策略即时重算**：修改升级天数或开启升级时，若存在未解除预警且触发已超期立即补发升级预警；关闭升级时清除排队中的升级任务 |
...
| C07 | **阈值修改重算置换** → 修改阈值保存后立即重算，若仍命中且有未解除预警，旧流水置「已作废」（原因：`预警配置阈值调整重算置换`）并生成新预警流水推送；若不命中则保留旧流水并进入新一轮判断 |
| C04~C06a | 贷中风控启用时幂等同步台账；关闭时标记不可执行 |
...
| V02 | 三类订单类型展示 | 切换订单类型筛选并新增抵押、质押、监管服务订单规则 | 筛选、列表、详情和新增订单选择器仅展示三类新类型；旧历史 `监管` 不展示且不映射（R12） |
| V03 | 重复配置订单 | 选择已配置规则的订单提交首次策略保存 | 保存阻断，提示订单已配置规则（R04） |
| V04 | 至少启用一项校验 | 编辑页关闭最后 1 项已启用策略并保存 | 保存阻断，提示请至少启用一项预警策略（R07a） |
| V05 | 独立通知精准触达 | 跌价与抵质押率分别配置不同预警对象 | 各子项触发时仅通知各自配置的预警对象，互不串扰 |
...
| 2026-09-15 | **V3.8** | **订单类型收敛**：统一为抵押、质押、监管服务；旧历史 `监管` 下线且不与监管服务兼容映射；同步策略矩阵、筛选、新增和验收口径 |
| 2026-09-15 | **V3.9** | **LTV 单等级对齐原型**：抵质押率子项改为双阈值单等级；列表摘要不再展示 `L3/L4` 式双等级聚合 | §2 功能范围、§3.4 策略矩阵、§4 列表/表单、§8 规则摘要、V05 |
| 2026-09-15 | **V4.0** | **LTV 分线解除与命中优先级**：补齐当前 LTV 展示、补仓/平仓分线解除方式、R13d/R13e | §3.4、§4.2、§6 S01 |
| 2026-09-15 | **V4.1** | **LTV 表格 UX**：策略 2 改为当前 LTV + 刻度条 + 双行表格 + 底部短提示，对齐 `LtvDualStrategyFields` 原型 | §4.2 |
| 2026-09-15 | **V4.2** | **LTV 文案与 Mock 对齐**：触发条件改为「若超过 X% 时触发预警」；无法计算提示紧邻当前率；Mock 补齐无 LTV 订单（DZY-R08a） | §4.2 |
| 2026-09-16 | **V4.3** | **H5 待处置红点口径**：通知渠道说明由「系统小角标」调整为 H5「押品预警信息」待处置红点自动展示；外部通知须配置对象并勾选渠道 | §2 功能范围、§4.2 各策略通知配置 |
| 2026-09-16 | **V4.3** | **H5 待处置红点口径**：通知渠道说明改为 H5「押品预警信息」红点自动展示；外部通知须配置对象并勾选渠道 | §2 功能范围 |
| 2026-09-18 | **V4.5** | **移动端待处置红点 + 升级渠道口径**：H5 统一为「移动端」；通知对象/升级配置仅勾选短信/邮件后展示；升级督办仅短信逐级上报 | §2 功能范围、§4.2 各策略通知配置 |

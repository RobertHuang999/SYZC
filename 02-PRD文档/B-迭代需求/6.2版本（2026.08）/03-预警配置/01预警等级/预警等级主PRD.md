...
> | **核心** | [预警等级字段清单](./预警等级字段清单.md) | 字段定义、筛选/列表/新增编辑/详情、展示顺序与控件规格 |
> | **核心** | [预警等级业务规则规格](./预警等级业务规则规格.md) | 状态机本体、R/C/P 规则、时序图、流转表 |
> | *衍生* | ASCII 线框 / Demo 文档 | 由三件套派生的可视化与原型输入，**非独立真源** |
>
> **版本**：V3.1 | **更新日期**：2026-09-11
> **读者**：研发工程师、测试工程师、配置管理员、风控架构师
> **所属模块**：03-预警配置 / 01-预警等级 | **菜单路径**：`预警配置 → 预警等级`
> **模板选型**：基础档案类（[5-2-1 基础档案类 PRD 模板](../../../../05-常用模板/需求处理/5-2-1%20(输出)基础档案类PRD模板.md)）
> **规则来源**：使用状态、校验、联动与权限详见《[预警等级业务规则规格](./预警等级业务规则规格.md)》——**规则 ID 以该文件为 唯一定义来源，本文仅摘要引用**
> **字段基准**：《[预警等级字段清单](./预警等级字段清单.md)》为字段唯一定义来源
> **菜单路径**：`预警配置 → 预警等级`
> **权限基准**：[00-全局契约与RBAC权限矩阵](../../00-全局契约与RBAC权限矩阵.md) 第二章、第四章 §1
> **跨模块契约**：[00-全局契约与RBAC权限矩阵](../../00-全局契约与RBAC权限矩阵.md)
> **跨文档引用规范**：[B-迭代需求跨文档引用口径与来源索引](../../../README.md)，本模块是 REF-SEVERITY 的唯一定义来源。
>
> **衍生文档（可视化 / 原型）**：
> - **ASCII 线框**：[列表页](./预警等级_ASCII_列表页.md) · [新增/编辑页](./预警等级_ASCII_新增编辑页.md) · [详情页](./预警等级_ASCII_详情页.md)
> - **Demo 输入**：[列表页](./预警等级_Demo_列表页.md) · [新增/编辑页](./预警等级_Demo_新增编辑页.md) · [详情页](./预警等级_Demo_详情页.md)
> - **原型 DEV**：PC [`/物联网IOT与预警/预警配置/预警等级`](http://localhost:5173/#/物联网IOT与预警/预警配置/预警等级)
...
> 本节描述**做什么、怎么交互**；字段名/控件见《[字段清单](./预警等级字段清单.md)》；线框见衍生 ASCII 文档。6.2 **无移动端写操作**。

### 4.1 PC 列表页

| 项目 | 说明 |
| :--- | :--- |
| 页面目标 | 查看、筛选、拖拽排序全部预警等级档位，进入新增/编辑/详情 |
| 页面路径 | `/预警配置/预警等级` |
| 骨架结构 | 页头 + 筛选 Card + 数据表格 + 底栏摘要（**无分页**） |

**筛选区**：默认行「是否启用」（全部/是/否）；Header 集成「+ 新增等级」（达 20 档 disabled，R07）。租户 2~20 档全量展示，无 Tab 栏。

**数据表格**：

- 左侧 `≡` 拖拽手柄调整 `sort_order`（R04/R12）；序号反映当前严重度排序
- 列：等级编码、显示名称、标签颜色、同步至订单预警、是否启用、等级说明、操作
- 编码/名称可点击进详情；**无行勾选、无分页**
...
删除/停用须满足 R08~R11；停用后 03/02、03/03 下拉隐藏该档（C03）。
...
**页头**：返回/取消（Dirty 确认）+「保存」（保存即生效，无审核流）。

**表单要点**（字段规格见字段清单）：

| 字段 | 交互要点 |
| :--- | :--- |
| 等级编码 | 新增可填；编辑可改；租户内唯一 |
| 显示名称 | 必填 |
| 标签颜色 | 色板选择 + HEX |
| 同步至订单预警 | Switch；控制 C02 穿透（R13） |
| 是否启用 | Switch；默认启用 |
| 等级说明 | 选填 |

**禁止项**：本页**不得**出现通知渠道、预警对象、升级天数/对象（R06）。
...
| 能力 | 只读展示全部字段 + 变更审计摘要；页头「编辑」跳转编辑页 |
...
> 以 [00-全局契约与RBAC权限矩阵](../../00-全局契约与RBAC权限矩阵.md) 第二章、第四章 §1 为准。
...
| 2026-08-20 | **V3.0** | **6.2 标杆重构**：对齐配置字典 PRD 章节（第1~10章）；In/排除范围 (Out Scope)；业务场景 S01~S06；启用/停用动作矩阵（非单据 SM）；权限/边界/验收 V01~V10；规则下沉至业务规则规格 唯一定义来源（R/C/P 体系） |
| 2026-09-11 | **V3.1** | **对齐三件套标杆**：补 §4 页面功能说明；三件套/衍生文头；章节重编号 |

...
> 点击「查看申请详情」→ 详情直达链接 跳转 PC「我的开锁申请」标签页 并打开详情（`ROUTE-IOT-APPR-01`）。
...
见 [获取门锁密码 ASCII（免审）](../../../01-物联网IOT管理/02门禁设备/门禁设备_ASCII_获取门锁密码.md) §1~§2：事由 → 备注（取消填写有效期）；成功态引导至【我的申请记录】，弹窗内不展示明文。
...
| 规则编号 | 场景 | 预期 |
| :--- | :--- | :--- |
| **【无获取密码权限 / 仓库数据权限】(R05)** | 无获取密码权限 / 仓库数据权限不足 | 阻断，提示无权限 |
| **【挂锁未绑定手机号】(R05)** | **挂锁**未绑定手机号 | 阻断，提示先绑定手机号 |
| **【未命中需审批却误入本弹窗】(R06)** | 未命中需审批却误入本弹窗 | 前端不应打开；后端兜底走免审 |
| **【挂锁存在在途申请或生效凭证】(R07)** | **挂锁**存在在途申请或生效凭证 | 阻断，提示该挂锁已有在途申请或生效凭证（阻止流程） |
| **【人脸同设备存在在途申请】(R07)** | **人脸**同设备存在在途申请 | 阻断，提示已有申请单号 |
| **【60 秒内重复提交】(R08)** | 60 秒内重复提交 | 返回原申请单号及状态（幂等） |
| — | 人脸有效期开始 > 结束 / 超 24h | 阻断，提示有效期不正确 |
...
| 2026-09-09 | **V1.4** | **挂锁取消有效期填写**，更新表单投影与校验规则，同设备再次申请走阻止流程 |

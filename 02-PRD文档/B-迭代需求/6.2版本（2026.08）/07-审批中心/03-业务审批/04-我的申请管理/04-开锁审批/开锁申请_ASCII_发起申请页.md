...
点击「获取门锁密码」/「获取门禁密码」后，**先匹配审批配置**（C01–C04 + 全局开关），再决定弹窗类型：
...
| **需审批** | 命中配置或全局审批开关 | **发起开锁申请**（本文档） | 提交申请 | 创建申请单，待审批 |
...
```text
         ...
         |    💡 仅供审批参考，不替代凭证有效期                               |
         |                                                              |
         |    💡 审批通过后请在申请详情页查看临时密码（**人脸不下发短信**）      |
         |                                                              |
         |                              [ 取消 ]  [ 提交申请 ]            |
         +--------------------------------------------------------------+
```

> 人脸专属字段（开锁次数、有效期）继承 [05获取门禁密码字段清单](../../../01-物联网IOT管理/02门禁设备/操作字段清单/05获取门禁密码字段清单.md)；审批增量字段「预计使用时段」与挂锁一致。
...
| 对比项 | 免审 | 需审批（本弹窗） |
| :--- | :--- | :--- |
| 触发条件 | 未命中审批配置 | 命中审批配置或全局审批开关 |
| 挂锁表单 | 事由、备注 | 事由、备注、**预计使用时段** |
| 人脸表单 | 事由、开锁次数、有效期、备注 | 同上 + **预计使用时段** |
| 主按钮 | 获取密码 | 提交申请 |
| 挂锁提交后 | 临时密码 + 有效期 + 短信结果 | 申请单号 + 待审批 + 凭证未生成 |
| 人脸提交后 | 临时密码卡片（无短信） | 申请单号 + 待审批 + 凭证未生成 |
...
- **共用表单区**：事由、备注（继承获取门锁密码字段清单）。
- **设备类型扩展区**：
  - **挂锁**：无额外字段（仅 helper 提示短信）；
  - **人脸**：开锁次数、有效期（继承获取门禁密码字段清单）。
- **审批增量区**：预计使用时段（必填，开始 ≤ 结束）。
- **底部动作**：取消 / 提交申请。

**提交后不可见字段**（服务端写入）：请求幂等键、设备与位置快照、审批配置快照、命中位置说明。
...
| — | 预计使用时段开始 > 结束 | 阻断，提示时段不正确 |
| — | 人脸开锁次数 / 有效期越界 | 同免审获取门禁密码校验 |

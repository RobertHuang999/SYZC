...
> **当前订单类型口径（2026-09-15）**：本模块订单类型统一为 `抵押`、`质押`、`监管服务`。新预警流水按三类订单的真实类型写入并展示；旧历史 `监管` 类型已下线，不在当前列表、筛选、详情或新触发链路展示，也不得映射为 `监管服务`。
...
| :--- | :--- | :--- | :--- |
| 解抵押/解质押/监管服务超时 | `ORDER_CONFIG` | 命中策略的 03/01 动态等级快照 | 对应订单办结或服务结案 |
...
| 抵/质押率异常 | `ORDER_CONFIG` | 命中策略的 03/01 动态等级快照（LTV 子项整卡单等级） | 按命中线展示对应解除方式（R13e） |
...
| 预警类型 | 标准模板（变量取触发快照） |
| :--- | :--- |
| 解抵押/解质押/监管服务超时 | `当前{订单类型}订单的{品类}-{规格}（{数量}{单位}）未在 {日期} 完成约定期限内的办结！（阈值{天}天）` |
| 价格下跌 | `抵/质押物价值下跌！货值已下跌超过{跌幅}%（阈值 {阈值}%）` |
| 盘点异常 | `盘点异常！经盘点发现货物异常，请及时核查处理！` |
| 巡检异常 | `巡检异常！【{巡检人}】未按时巡检，请及时核查处理！` |
| 抵/质押率异常 | `订单抵/质押率异常！触发【{补仓线/平仓线}】，当前抵/质押率 (LTV) {值}%，贷款余额 {额} 元，质物价值 {值} 元` |
...
| **06** | **解抵押/解质押/监管服务超时** | 对应订单履约/服务期限 | 逾期 15 天 | 约定期限: 2026-06-01 | 抵押/质押未完成解押，或监管服务未完成结案 |
...
| 已结案 · 有效 · 未公示（抵/质押） | 公示风险、详情 |
| 已结案 · 有效 · 已公示 / 已取消 | 查看公示、详情 |
| 已结案 · 有效 · 监管服务 | 仅详情（不可发起公示） |
| 已作废 | 详情 |
...
| 订单与仓储快照 | 订单类型（抵押/质押/监管服务）、货主、`cargoItems[]` 逐行对齐「货物位置」与「大类/品类/规格-数量」 |
| 预警事实 | 预警内容（§3.4 模板）、触发时间、抓拍图、触发判定数据快照（4 列不可变存证）；**不**单独展示 LTV 命中快照卡片（ltvHitSnapshot 仅供解除引导弹窗 R13e） |
...
| 筛选 | 顶部搜索框 placeholder「请输入预警订单号」，**仅按订单号模糊匹配**；胶囊：类型、等级、状态（默认待处置 · 有效）、来源；抽屉：时间等 |
| 列表形态 | 卡片：订单号、等级、类型、来源、内容摘要、时间、状态；底链按 PC 行操作矩阵 |
| 详情 | 页头摘要灰框 + 折叠手风琴（触发快照/穿透/处置/无效） |
| 批量公示 | 进入勾选模式 → 仅已结案 · 有效且未公示可选（已公示复选框禁用）→ Bottom Sheet 确认 → `/batch-publish` 逐条编辑同款表单 → 提交全部（C12 / RISK-PUB-B03/B04） |
...
| S05 | 配置删除/子项关闭置无效 | **支线** | 01 整单软删除或子项关闭 → 关联待处置置「已作废」；系统自动失效不置无效 |
| S05a | 配置阈值修改重算置换 | **支线** | 03/03 配置修改阈值后触发重算：若仍命中新阈值且存在未解除预警，旧流水置「已作废」（原因：`预警配置阈值调整重算置换`）并生成新流水推送；若不命中则保留旧流水 |
...
| 已作废 | 被阈值调整重算置换、子项关闭、整单删除、订单办结或同类型覆盖，只读 | **是** |
...
stateDiagram-v2
    [*] --> PENDING: 配置命中或穿透生成
    PENDING["待处置 · 有效"] --> INVALID["已作废"]: 阈值重算置换/子项关闭/整单删除/订单办结/被覆盖
...
| 2026-09-15 | **V3.6** | **订单类型收敛**：统一展示抵押、质押、监管服务；旧历史 `监管` 下线且不与监管服务兼容映射；同步列表、详情和超时模板 |
| 2026-09-16 | **V3.7** | **详情页移除 LTV 独立卡片**：PC/H5 详情不展示 ltvHitSnapshot；命中线/阈值见预警内容与触发判定四列表；解除弹窗保留 R13e |

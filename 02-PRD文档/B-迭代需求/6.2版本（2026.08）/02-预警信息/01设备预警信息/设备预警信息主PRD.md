...
**设备预警信息**是仓储监管现场物理硬件异常告警事件的**统一承接与处置中枢**（`iot_event_ledger`）。门禁通行事务仍由门禁设备事务记录承接，不单独新增设备预警大类。

本模块承接原系统「设备预警信息（越界/离线/破坏）」及设备事务通知中的风险事件，预警类型严格限制为 5 大类。6.2 取消历史旧菜单【设备管理→门禁事务记录】；门禁设备「设备数据→事务记录」Tab 仍保留，通行事务按原门禁事务口径查询，不作为新的设备预警类型。
...
- **穿透监管目标**：设备风险告警与运维通知统一进入设备预警台账；门禁通行事务沿用门禁设备事务记录入口，边界清晰可追溯。
...
- 现场抓拍预览、超时升级通知、门禁风险事件映射、订单穿透与联动解除
- 门禁设备「设备数据→事务记录」Tab 只读，通行事务不新增设备预警类型
- PC/H5 列表、详情、单条解除
...
| 智能挂锁预警 | 03/02 规则命中及挂锁风险事件 | L2~L5 | 破坏类 → 人工；可自动结案的事件按处置策略执行 |
| 人脸门禁预警 | 03/02 规则命中及门禁风险事件 | L2~L5 | 异常 → 待处置；可自动结案的事件按处置策略执行 |
| 设备GPS预警 | 03/02 规则命中 | 按规则快照 | 围栏/超速 → 自动恢复；非法拆除 → 人工 |

> 配置侧与信息侧均为 5 大类；门禁 15 种事务名称仍按原门禁事务文档维护，不作为设备预警大类或级联筛选项。设备预警子类型与处置策略矩阵见《[设备预警信息业务规则规格](./设备预警信息业务规则规格.md)》附录 A。

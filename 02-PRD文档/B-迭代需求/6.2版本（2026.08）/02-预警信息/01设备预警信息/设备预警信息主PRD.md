...
| 下游影响 | **正式命中**：写入 `iot_event_ledger` 设备预警流水，包含事件 ID、设备/仓库/库房/分区快照、预警类型与子类型、首次/最近预警时间、预警次数、采集值与阈值、规则 `Version`、防抖参数、03/01 等级快照及通知/升级快照；待处置 · 有效期间重复事件按设备+子类型聚合并更新次数和时间，不重复发送首次短信。**通知与升级**：首次命中按规则快照通知，升级按 `upgrade_days` 和首次预警时间执行，流水已结案 · 有效/已作废或规则删除时取消待执行升级；发送失败进入补偿队列，不回滚流水。**订单穿透**：等级 `sync_to_order_warn=true` 且匹配在押订单时，向 02/02 投递 `IOT_PENETRATION`，以设备事件 ID 幂等；无匹配订单只保留设备流水。**解除联动**：设备流水解除后发布至少一次投递的 `DeviceEventReleased`（含租户、设备事件、订单、处理人和时间），供 02/02 按事件 ID 幂等联动解除；消费失败由补偿重放。来源：[设备预警信息业务规则规格](./设备预警信息业务规则规格.md) R01~R04、C01~C06、[跨文档引用口径与来源索引](../../../README.md) REF-SEVERITY/REF-NOTIFY。 |
| 实体关系 | 一条规则可产生多条预警流水（1:N）；同一设备+子类型待处置 · 有效期间仅 1 条有效聚合记录 |
...
| 关系 | 说明 |
| :--- | :--- |
| 设备预警配置 : 设备预警信息 | **1:N**（一条规则可产生多条流水；删除规则置待处置流水无效，见配置侧 C01） |
| 设备 + 子类型 : 待处置流水 | **1:1**（待处置 · 有效期间同类事件聚合至同一记录，C01） |
...
flowchart TD
    ...
    CHK -->|瞬态 R06| INST[立即落账 Count=1<br/>已结案 · 有效]
    FIRE --> EXIST{存在待处置 · 有效?}
...
| S02 | 待处置 · 有效期间频次累加 | **主流程** | 同设备同类型持续超标 → Count+1 → 更新最近时间与快照 → 不发短信 |
| S03 | 人工解除整轮归档 | **主流程** | 累计 N 次待处置 → 提交情况说明 → 联动抓拍 → 已结案 · 有效 → 下轮从 1 起算 |
| S04 | 自动恢复解除 | **主流程** | 温湿度恢复稳定 1 分钟 → 系统自动已结案 · 有效，处理人=系统自动处理 |
| S05 | 瞬态通行立即落账 | **主流程** | 刷脸通行/开锁成功 → 防抖=0 → Count=1 → 直接已结案 · 有效 |
| S06 | 超时升级通知 | **支线** | 待处置 · 有效 + upgrade_days>0 → 首次预警时间+N 天 → 向升级对象推送 |
| S07 | 规则删除置流水无效 | **支线** | 配置侧删除规则 → 关联待处置流水置已作废 → 升级终止 |
...
| 状态 | 含义 | 是否终态 |
| :--- | :--- | :---: |
| 待处置 · 有效 | 告警有效待处置，可聚合 Count、可人工/自动解除 | 否 |
...
```mermaid
stateDiagram-v2
    [*] --> 待处置_有效: 防抖满足首次触发 Count=1
    待处置_有效 --> 待处置_有效: 同类超标 C01 Count+1
    待处置_有效 --> 已作废: 规则删除/设备移除
    待处置_有效 --> 已结案_有效: 自动恢复 C03
    待处置_有效 --> 已结案_有效: 人工解除 R04
    已作废 --> [*]: 归档只读
    已结案_有效 --> [*]: 归档只读 再次超标新轮次 C04
```
...
| **R14'** | 解除入口以触发快照 `disposition_mode=ACTION_REQUIRED` 为准（对齐 03/02 R15b）；非子类型白名单门控 |

#### 7.3 频次聚合与升级

| 规则ID | 规则摘要 |
| :--- | :--- |
| C01 | 待处置 · 有效期间 Count+1，不重复发短信；聚合键=设备+子类型（不含阈值/通知人变更） |
| C04 | 已结案 · 有效归档后，再次超标全新轮次 Count 从 1 起算 |
...
| 规则已删除，流水仍显示待处置 · 有效 | 配置侧 C01 置已作废 |
...
| V03 | 人工解除归档 | 累计 12 次待处置，提交一次人工解除 | 整体变已结案 · 有效，下次超标 Count 从 1 起 |
| V04 | 等级筛选 | 开锁 L2、超温 L4 | 筛选 L4 仅超温，L2 仅开锁 |
| V05 | 超时升级 | 规则升级 3 天，流水待处置 · 有效 | 第 3 天推送升级；解除后停止；Count 不重置计时 |
| V06 | 自动恢复 | 温湿度恢复稳定 1 分钟 | 状态=已结案 · 有效，处理人=系统自动处理 |
| V07 | 瞬态通行 | 刷脸通行成功 | Count=1，直接已结案 · 有效，不发短信（默认） |
| V08 | 规则删除联动 | 配置侧删除含待处置流水的规则 | 流水置已作废，升级终止 |

...
| :--- | :--- |
| 写操作名称 | 解除预警 |
| 页面目标 | 人工核销待处置 · 有效告警，提交解除材料并归档整轮触发 |
...
**辅助提示**（虚线边框区块）：确认解除后将归档整轮 {N} 次触发，状态变为「已结案 · 有效」。
...
**不可进入条件**（R14'）：仅自动恢复类型、已结案 · 有效、已作废 → 不展示入口或路由拦截 + Toast。
...
| R11 非待处置 · 有效 | Toast「该预警已结案或已失效」 |
| R12 Version 冲突 | Toast「数据已被他人修改，请刷新后重试」 |
| 非 ACTION_REQUIRED | 不展示解除入口 |

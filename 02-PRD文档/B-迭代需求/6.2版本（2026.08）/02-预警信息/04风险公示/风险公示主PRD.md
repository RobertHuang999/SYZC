...
### 2.1 本期范围
- **公示列表检索**：PC 支持规则名称、订单号、货主、预警类型四字段独立模糊匹配（AND 组合）；H5 为搜索字段下拉 + 关键词单字段模糊匹配；列表固定仅展示最新状态为“已公示”的记录（RISK-PUB-ST02）；
...
### 2.2 移动端范围
- **风险公示台账**（`/m/risk/disclosures`）：列表查询 + 详情快照查看；详情底部与 PC 对齐提供【取消公示】【重新公示】（需关联押品预警 ID）；取消说明必填 ≤200 字。
- **押品预警侧公示**（`/m/supervision/order-warnings`）：支持单条「公示风险」确认页、列表「批量公示」勾选模式，以及押品路径下的公示详情（同款快照表单、逐条独立落库）。
...
**筛选区（PC）**：规则名称、订单号、货主、预警类型四个独立模糊搜索框，条件按 AND 组合；列表固定仅展示**已公示**记录（RISK-PUB-ST02），不提供公示状态或公示时间日期范围筛选项。

**筛选区（H5）**：搜索字段下拉（规则名称/订单号/货主/预警类型）+ 关键词输入框，仅对所选字段模糊匹配；不提供公示时间日期范围筛选项。

**数据表格**：

- 列：序号、预警订单号、预警类型、公示标题与内容摘要、现场抓拍图、最近一次公示时间、最新操作人、公示状态、操作
...
| 入口 | 能力 |
| :--- | :--- |
| 风险公示台账列表 | 仅展示已公示副本（RISK-PUB-ST02）；H5 为字段下拉 + 关键词单字段模糊匹配 |
| 风险公示台账详情 | 快照详情 + 操作审计；底部【取消公示】【重新公示】与 PC 对齐 |
| 押品预警 · 单条公示风险 | 首次确认页（快照表单）→ 提交生成 V1 已公示记录 |
| 押品预警 · 批量公示 | 勾选模式 → Bottom Sheet 确认 → 批量公示确认页逐条编辑 → 提交全部 |
| 押品预警 · 公示详情 | 与台账详情同款快照与底部操作区 |
...
**取消公示 Bottom Sheet**：固定合规文案 + 取消说明必填 ≤200 字（PUB-TC05）；成功后列表默认筛选隐藏该项，详情保留审计。

> 重新公示跳转押品预警公示确认页（`republish=true`）；无关联押品预警 ID 的历史记录仅支持查看，不展示写操作按钮。
...
| 2026-09-17 | **V2.1** | 产品团队 | **移动端风险公示详情补齐取消/重新公示**：台账详情底部与 PC、押品侧对齐；更新 §2.2 / §4.4 移动端范围 |

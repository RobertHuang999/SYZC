...
### 2.1 本期范围
- **公示列表与状态检索**：支持按规则名称（模糊）、预警类型（模糊）、订单号（模糊）、货主名称（模糊）、状态（当前默认仅展示最新状态为“已公示”的记录）多维度筛选；
...
### 2.2 移动端范围
- **风险公示台账**（`/m/risk/disclosures`）：列表与详情只读查看、取消公示不在移动端提供。
- **押品预警侧公示**（`/m/supervision/order-warnings`）：支持单条「公示风险」确认页与列表「批量公示」勾选模式（与 PC 同款快照表单、逐条独立落库）；取消公示仍在 PC 风险公示详情完成。
...
**筛选区**：规则名称、预警类型、订单号、货主名称（模糊匹配）；预警类型为单行文本输入框（公示确认页可编辑类型文案，故不做下拉枚举）；状态下拉（6.2 列表默认仅展示最新状态=**已公示**）。
...
| 入口 | 能力 |
| :--- | :--- |
| 风险公示台账列表/详情 | **只读**查看已公示副本与操作审计；列表支持预警类型模糊匹配筛选；取消公示在 PC 完成 |
| 押品预警 · 单条公示风险 | 首次确认页（快照表单）→ 提交生成 V1 已公示记录 |
| 押品预警 · 批量公示 | 勾选模式 → Bottom Sheet 确认 → 批量公示确认页逐条编辑 → 提交全部 |

> 移动端不提供风险公示台账侧的「取消公示」；已取消记录从押品侧「公示风险」直达只读公示详情（PUB-TC01）。

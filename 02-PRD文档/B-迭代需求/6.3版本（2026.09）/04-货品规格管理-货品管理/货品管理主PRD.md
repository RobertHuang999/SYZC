...
> **模块定位**：货品规格管理第 3 节点（SKU 主档），作为存货监管最小业务与交易核算单元，维护动态规格参数取值、计量参数与单位、入库收集信息配置（逐项必填）、基准参考单价（元/万元精度）与设价日志，实现自身/有效状态双层控制、入库强校验与两阶段安全删除校验。  
> **版本**：V2.0 基准版 | 更新时间：2026-09-16 | 责任人：森云科技（Senyun Technology）  
> **编写依据**：[货品管理字段清单.md](./货品管理字段清单.md) 是本模块所有字段编码、格式、16 项入库收集预置项与页面展示的唯一定义真源；[货品管理业务规则规格.md](./货品管理业务规则规格.md) 是本模块状态机、有效启用公式、两阶段删除与设价规则的唯一定义来源。

---

## 1. 业务背景

货品管理是 SYZC 存货监管系统“货-仓-金”全链路的最小实体基石。货品（SKU）继承品类所定义的动态规格维度，并绑定计量单位、入库信息收集规则和参考单价，向仓储入库单、实时库存台账、数字仓单、动产质押与融资核算提供唯一的货品规格与价格参照。
...
| :--- | :--- |
| - 货品列表与左侧大类/品类层级树联动展示；<br/>- 添加货品（填写规格取值、计量参数与单位）；<br/>- 入库收集信息配置（从系统 16 项预置项中勾选并逐项配置必填）；<br/>- 编辑货品与入库配置（入库单引用强拦截）；<br/>- 启用/停用货品与有效状态实时多级动态计算；<br/>- 设置价格（支持元/万元双精度展示）与设价历史流水留痕；<br/>- 货品两阶段删除（阶段一启用拦截 + 阶段二入库单扫描）；<br/>- 货品详情全量只读预览与操作审计追溯。 | - 入库单、出库单业务办理与实物质检；<br/>- 数字仓单生成、流转与解押办理；<br/>- 融资授信审批与额度扣减；<br/>- 大宗商品外部行情自动拉取与自动调价。 |
...
| **场景 2 (SKU-S02)** | 【入库收集信息配置与必填校验】 | 主流程 | 从 16 项系统预置项中按需勾选采集字段，逐项配置是否必填；入库单选择该货品后动态渲染对应字段并强校验。 | 【入库收集信息预置项配置规则】(SKU-F03)<br>【入库单动态联动与强校验规则】(SKU-F04) |
...
| **SKU-F03** | 【入库收集信息预置项配置规则】 | 从 16 项预置项中勾选并逐项设必填；未配置入库单不展示额外采集字段 | [业务规则规格 SKU-F03](./货品管理业务规则规格.md#入库收集信息预置项配置规则sku-f03) |
| **SKU-F04** | 【入库单动态联动与强校验规则】 | 入库单动态渲染采集项；强校验必填、数值区间与日期逻辑关系 | [业务规则规格 SKU-F04](./货品管理业务规则规格.md#入库单动态联动与强校验规则sku-f04) |
...
| **SKU-F08** | 【编辑操作入库单强拦截规则】 | 编辑前实时扫描入库单引用；存在任何入库数据阻断修改并弹窗提示 | [业务规则规格 SKU-F08](./货品管理业务规则规格.md#编辑操作入库单强拦截规则sku-f08) |
...
| **SKU-V03** | 【入库收集项必填校验测试】 | 货品配置了必填入库项，在仓储入库单提交时未填报该项 | 前后端均阻断提交，提示对应字段必填 |
...
| **2026-09-16** | **V2.0 规范化升级** | 1. 编号语义自解释：场景命名为 `场景 N【业务场景名称】(SKU-S01~S06)`，用例命名为 `【用例业务名称】(SKU-V01~V09)`；<br>2. 交互术语全面汉化：Toast/Modal/Alert 全面汉化为浮窗提示、模态弹窗、模态警告弹窗；<br>3. 强化与三件套真源强对齐。 | 森云科技规范化升级工作组 |

...
| :--- | :--- |
| - 列表多维检索（转让单号、日期范围、货主、仓库、货品、制单人、制单时间）与 10 列展示（操作列仅查看详情）；<br/>- 新增普通存货转让申请：转让方三要素联动、500 字必填转让原因说明、树形货品明细及实时合计；<br/>- 选货弹窗（单仓强阻断沙箱、部分转让拆分）；<br/>- 新接收方二元身份核验（6位短信验证码、60s防刷）与自动建档至货主档案；<br/>- 审批流转（审批通过 / 驳回 / 撤回）与库存锁定；<br/>- 跨模块【转让入库】无缝联动：终审通过后原子生成受让方转让入库记录，条码资产归属原子变更（储位物理不动）；<br/>- 转让详情页纯净展示（6 项基础信息网格 + 双 Tab，无现场照片与设备抓拍）。 | - 质押/抵押货物直接转让（必须解押转自有后方可发起）；<br/>- 现场照片与设备抓拍（转让为纯货权交易，货物物理不移动）；<br/>- 单据下载与下载记录（操作列仅提供查看详情）；<br/>- 草稿箱与暂存（未提交退出即丢弃）；<br/>- 已归档单据反冲正（已生效单据禁止反审批撤回）；<br/>- 筛选区重置按钮（遵循系统统一规范）。 |
...
```mermaid
flowchart LR
    A[转让方 / 出让货主] --> D[转让记录单据<br/>PRO 申请]
    B[自有在库可用库存] -->|校验可用量并锁定| D
    C[接收方 / 受让货主] -->|未建档新客户二元短信核验| D
    D -->|提交审核| E[审批中心]
    E -->|审批通过| F[WMS 库存与货权系统<br/>原子货权对冲]
    F -->|原货主: 转让出库记录| G[出库管理]

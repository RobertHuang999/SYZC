...
- “设备数据”弹窗只读展示门禁“事务记录”（开关锁事件日志）与“预警记录”（开锁异常/门锁超时未闭合等预警）；其中“事务记录”Tab与【设备管理 - 门禁事务记录】独立菜单共用同一数据集，前者按当前设备上下文展示，后者支持全量筛选。
- 挂锁门禁【获取门锁密码】的安全验证、提示弹窗、事由/备注填写、手机号短信下发及页面密码展示；
- 人脸门禁【获取门禁密码】的安全验证、配置弹窗（事由、开锁次数最大100次、最大24小时有效期、备注）、手机号短信下发及页面“临时开锁密码”卡片展示与一键复制；
...
列表字段、筛选字段、字段来源、取值、展示顺序和字段级校验统一以[门禁设备字段清单](./门禁设备字段清单.md)为准；本主 PRD 不重复定义字段属性。行操作的输入、查询、展示、返回和特有审计字段统一以[门禁设备操作字段清单索引](./操作字段清单/操作字段清单索引.md)及其子清单为准。事务记录和预警记录字段分别由关联模块维护。
...
| :--- | :--- | :--- | :---: | :--- |
| 重命名 | 挂锁门禁、人脸门禁 | 列表“操作”列 | 是 | 继承[监控设备主 PRD·5.2 重命名](../16-设备管理-监控设备/监控设备主PRD.md#52-重命名) |
| 仓库绑定 | 挂锁门禁、人脸门禁 | 列表“操作”列 | 是 | 继承[监控设备主 PRD·5.3 仓库绑定](../16-设备管理-监控设备/监控设备主PRD.md#53-仓库绑定) |
| 设备数据 | 挂锁门禁、人脸门禁 | 列表“操作”列 | 否 | 本文档第5.4节；其中“事务记录”Tab与【设备管理 - 门禁事务记录】独立菜单共用同一数据集，入口和展示容器不同 |
| 获取门锁密码 | 仅挂锁门禁 | 列表“操作”列 | 否 | 本文档第5.5.2节（挂锁门锁密码，展示提示弹窗并下发短信密码，有效期3天） |
| 获取门禁密码 | 仅人脸门禁 | 列表“操作”列 | 否 | 本文档第5.5.3节（人脸门禁临时开锁密码，可配置次数与最大24h有效期，带【复制】按钮卡片） |
| 人脸配置 | 仅人脸门禁 | 列表“操作”列 | 否 | 本文档第5.6节（当前页面平铺展示有人脸配置用户，无用户时点击【前往管理页面 ❯】跳转） |
| 移除设备 | 挂锁门禁、人脸门禁 | 列表“操作”列 | 是 | 继承[监控设备主 PRD·5.7 移除设备](../16-设备管理-监控设备/监控设备主PRD.md#57-移除设备) |

重命名、仓库绑定和移除设备的弹窗、权限、校验、并发控制、幂等和审计基础规则继承监控设备；门禁设备的跨模块移除结果处理和后续补偿以本PRD第8.4节及“后续可优化事项”为准。如监控设备基础规则发生变更，门禁设备同步继承最新版本。

### 5.2 重命名

继承[监控设备主 PRD·5.2 重命名](../16-设备管理-监控设备/监控设备主PRD.md#52-重命名)，重点规则如下：
...
继承[监控设备主 PRD·5.3 仓库绑定](../16-设备管理-监控设备/监控设备主PRD.md#53-仓库绑定)，重点规则如下：
...
挂锁门禁与人脸门禁均支持“获取密码”类行操作，但两类设备的前端展示页面、配置参数与临时凭证形态存在明确差异：

| 设备类型 | 行操作名称 | 核心页面交互差异 |
| :--- | :--- | :--- |
| **挂锁门禁** | 【获取门锁密码】 | 打开“获取门锁密码提示”弹窗，填写必填事由与选填备注，确认后向手机下发短信并在页面弹窗中展示门锁动态密码（固定有效期 3 天） |
| **人脸门禁** | 【获取门禁密码】 | 打开“获取门锁密码提示”/“获取门禁密码提示”配置弹窗，配置事由、开锁次数（最大100次）、有效期（最大24小时）及备注，确认后向手机下发短信并在页面弹出“临时开锁密码”卡片（含6位系统唯一数字密码及【复制】按钮） |

#### 5.5.2 挂锁门禁【获取门锁密码】规则

1. **入口与权限**：仅设备类型为“挂锁门禁”的行展示【获取门锁密码】；登录账号需具备“获取门锁密码”角色权限。
2. **字段引用**：提示内容、事由、备注、短信下发返回、动态密码展示和特有审计字段以[获取门锁密码字段清单](./操作字段清单/04获取门锁密码字段清单.md)为准，本主 PRD 不重复定义字段属性。
3. **业务规则**：确认后自动向登录账号绑定手机号发送短信，短信成功后展示动态密码及有效期；弹窗关闭后页面不留存明文。

#### 5.5.3 人脸门禁【获取门禁密码】规则

1. **入口与权限**：仅设备类型为“人脸门禁”的行展示【获取门禁密码】；登录账号需具备“获取门禁密码”角色权限。
2. **字段引用**：提示内容、事由、开锁次数、有效期、备注、短信下发返回、临时密码展示和特有审计字段以[获取门禁密码字段清单](./操作字段清单/05获取门禁密码字段清单.md)为准，本主 PRD 不重复定义字段属性。
3. **业务规则**：按字段清单完成表单校验后生成 6 位系统唯一数字密码并发送短信；成功后展示临时密码卡片及复制操作，弹窗关闭后页面不留存明文。

#### 5.5.4 密码审计与安全要求

- 当前系统允许具备权限的用户在页面点击眼睛图标查看开锁密码明文；该查看行为必须执行服务端权限校验并记录审计日志。
- 审计日志记录操作人、角色、设备编码、设备类型、事由、开锁次数（人脸门禁）、生效与到期时间（人脸门禁）、备注、请求标识、短信下发结果及操作时间；审计日志和数据库运行日志不得记录密码明文或短信具体内容。
- 密码追溯使用受权限保护的密码凭证及操作记录进行匹配，不以列表行号作为关联键；具体存储方案由技术设计补充，但不得影响已授权页面明文查看能力。
...
| 2 | 门锁与门禁动态密码短信模板配置 | 挂锁密码有效期固定为3天；人脸门禁密码为6位系统唯一数字且有效期最大24小时 |
| 3 | 门禁设备平铺授权的终端失败处理 | 已确认：下发失败即最终失败，Switch回滚，不离线等待或自动重试 |
| 4 | 门禁设备实时状态（在线/离线）的轮询或推送机制 | 设备状态显示与设备状态筛选 |
| 5 | 跨模块移除的回滚任务状态与失败告警 | 业务结果按原子语义处理；补偿任务仅作为后端回滚失败时的内部实现，设备保持原状态且不得返回成功 |
| 6 | 人脸门禁移除时人脸配置授权关联处理 | 已确认：同步删除授权关系及“门禁数据”关联；5分钟轮询重入不恢复授权和凭证 |
...
- **顶部管理跳转**：页面左上方标题右侧提供红字引导提示：“❓ 未找到可分配的用户 前往管理页面 ❯”。仅在此处点击【前往管理页面 ❯】按钮/链接时，才真正携带当前设备上下文跳转至 [设备管理/人脸配置/人脸管理](../21-设备管理-人脸配置/人脸配置主PRD.md) 主页面。
- **筛选区**：提供“人员名称”输入框以及【查询】和【重置】按钮，支持按人员姓名进行模糊搜索。

#### 5.6.2 列表字段与展示规则

平铺视图的列表、筛选、账号脱敏、授权状态和操作结果字段统一以[`人脸配置字段清单.md`](../21-设备管理-人脸配置/人脸配置字段清单.md)为准；本主 PRD 仅定义当前设备上下文下的授权准入、排序、权限、确认、下发和失败回滚规则。
...
3. **授权开关交互**：鼠标悬停操作列问号图标展示提示气泡：“开启/关闭 用户用于开门的权限”。切换 Switch 开关时弹窗二次确认，确认后发起下发/撤销凭证请求。下发结果为直接返回（下发失败即最终失败，设备离线或下发异常直接返回失败，系统不进行持续离线等待与上线自动重试），失败时 Switch 状态自动回滚并提示：“授权下发失败，请检查门禁设备网络”。用户如需重新授权需手动重新点击切换。
4. **常规分页**：列表底部提供完整的分页控件（支持总条数、页码切换、每页条数下拉及跳页）。
5. **权限边界**：人员列表按登录账号所属机构数据权限过滤；当前门禁设备的授权开关和分配操作按设备绑定仓库数据权限过滤。两类权限均需由服务端独立校验。

### 5.7 移除设备

继承[监控设备主 PRD·5.7 移除设备](../16-设备管理-监控设备/监控设备主PRD.md#57-移除设备)，重点规则如下：
...
| 获取门锁密码 | 是 | 按设备绑定规则 | 专属角色权限（挂锁门禁） | 可发起获取并发送挂锁短信密码（页面弹窗显示，有效期3天） |
| 获取门禁密码 | 是 | 按设备绑定规则 | 专属角色权限（人脸门禁） | 可配置事由/次数/有效期并下发短信，页面弹出带【复制】按钮的6位临时密码卡片 |
...
- 获取门禁密码（人脸门禁：记录事由、开锁次数、生效时间、到期时间、备注及短信发送结果，**严禁记录密码明文**）；
- 人脸配置授权开关变更；
- 移除设备及其必填说明；
- 第三方门禁接口调用失败。
...
| 人脸门禁临时密码 | 密码服务 / 短信网关 | 根据配置的开锁次数（最大100次）与有效期（最大24小时）动态生成6位系统唯一数字密码，短信下发并在页面弹窗中展示与一键复制，数据库及日志严禁存储明文 |
...
- **短信发送失败**：提示：“密码发送失败，请稍后重试或联系管理员”，不展示明文/临时密码弹窗，不更新操作成功状态。
- **未绑定手机号**：若登录账号未绑定手机号，提交时阻止请求，提示：“当前登录账号未绑定手机号，无法获取开锁密码”。
- **人脸门禁表单校验失败**：未选择事由、开锁次数非1~100整数、有效期超过24小时或备注超过50字时，前端阻断提交并给出精准提示（如：“密码有效期最大不得超过 24 小时”）。
...
sequenceDiagram
    ...
    alt 校验不通过
        ...
        F->>B: 提交门锁密码获取请求（设备编码、事由、备注）
        B->>B: 频率防刷与账号手机号校验
        B->>SMS: 请求下发门锁密码短信
        SMS-->>B: 返回短信下发结果及动态密码
        B->>L: 写入获取记录审计（包含事由备注，严禁明文密码）
        B-->>F: 返回成功结果与展示密码
        F-->>U: 页面弹窗展示门锁密码（提示有效期3天）
    end
```
...
sequenceDiagram
    ...
    alt 校验不通过
        ...
        B-->>F: 打开“获取门禁密码提示”配置弹窗
        U->>F: 填写事由(必填)、开锁次数(1-100次)、有效期(最大24h)及备注并点击确认
        F->>B: 提交门禁密码获取请求（设备编码、事由、开锁次数、开始/结束时间、备注）
        B->>B: 执行表单校验、防刷与获取当前登录账号绑定的手机号
        B->>SMS: 生成6位系统唯一数字密码并请求短信下发
        SMS-->>B: 返回短信下发结果及6位临时密码
        B->>L: 写入获取记录审计（含事由、次数、生效与到期时间、备注，严禁明文密码）
        B-->>F: 返回成功结果、6位密码、开锁次数与有效期
        F-->>U: 页面弹出“临时开锁密码”卡片（展示6位密码、次数、有效期及【复制】按钮）
    end
```
...
| G-04 | 获取门锁密码 | 有对应敏感权限的人员 | 挂锁门禁事由、备注、短信和密码展示 | 获取密码 | 密码有效期3天，禁止记录明文 |
| G-05 | 获取门禁密码 | 有对应敏感权限的人员 | 人脸门禁事由、次数、有效期、临时密码 | 获取密码 | 次数1～100，最长24小时，禁止记录明文 |
...
- 行操作字段见[操作字段清单索引](./操作字段清单/操作字段清单索引.md)，人脸授权完整规则见[人脸配置主PRD](../21-设备管理-人脸配置/人脸配置主PRD.md)。
...
| A08 | 挂锁门禁点击【获取门锁密码】弹出提示弹窗并校验必填事由与角色权限；人脸门禁点击【获取门禁密码】弹出配置弹窗，校验必填事由、开锁次数（1~100次整数，默认1）、有效期（开始时间默认当前，结束时间距开始时间最大24小时）及角色权限 |
| A09 | 挂锁门禁与人脸门禁备注最多均只允许输入 50 个字，超过时前端拦截并提示 |
| A10 | 人脸门禁点击确认后自动发送短信给登录手机号，同步在页面弹出“临时开锁密码”窗口展示6位系统唯一数字密码、开锁次数与有效期区间，并提供一键【复制】功能 |
...
| :--- | :--- | :--- |
| 1 | 设备编码生成方、编码格式、长度及接口字段编码 | 门禁设备基础字段、维护接口和幂等关联 |
| 2 | 门锁与门禁动态密码短信模板配置 | 可优化项；挂锁密码有效期固定为3天，人脸门禁密码为6位系统唯一数字且有效期最大24小时 |

...
| （列表固定仅展示已公示记录；不提供公示状态或公示时间日期范围筛选项）                                  |
+----------------------------------------------------------------------------------------------------+
...
1. **筛选工作区**：
   - **规则名称 / 订单号 / 货主 / 预警类型**：四个独立单行文本输入框，各自模糊匹配，条件按 AND 组合；
   - **列表范围**：固定仅展示已公示记录（RISK-PUB-ST02），不提供公示状态或公示时间日期范围筛选项；
...
2. **表格工作区与行级操作列**：
   ...
   - **行级操作按钮**：
     ...
     - **【重新公示】**：已公示或已取消且存在关联押品预警 ID 时展示，跳转押品预警公示确认页（`republish=true`）。

### 2.3 移动端 H5 台账与详情（`/m/risk/disclosures`）

| 页面 | 能力 |
| :--- | :--- |
| 列表 | 仅展示已公示副本（RISK-PUB-ST02）；搜索字段下拉 + 关键词，单字段模糊匹配 |
| 详情 | 公示状态 + 公示信息/处置信息快照 + 操作记录 |
| 详情底部 | 与 PC 对齐：已公示展示【重新公示】【取消公示】；已取消展示【重新公示】 |

**取消公示 Bottom Sheet**：展示 `【取消公示固定合规二次确认文案规则】(RISK-PUB-C02)` 固定文案 + 取消说明多行文本（1~200 字必填）；提交成功后列表隐藏、详情保留审计（PUB-TC06）。

> 无关联押品预警 ID 的历史兼容记录仅支持查看，不展示写操作按钮。
...
| **V2.2** | 2026-09-17 | 产品团队 | 新增 §2.3 移动端 H5 台账详情；补齐【取消公示】【重新公示】底部操作区，与 PC 及押品侧对齐。 |
| **V2.3** | 2026-09-17 | 产品团队 | PC 四字段 AND 模糊筛选 + H5 字段下拉检索；移除公示时间日期范围筛选；列表固定仅展示已公示（RISK-PUB-ST02）。 |

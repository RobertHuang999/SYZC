...
>
> **当前订单范围**：监管订单当前关闭。当前新预警、订单处置和风险公示仅适用于抵押/质押订单；历史监管预警仅支持按权限查询、详情、审计和资料查看，不提供解除、公示或其他业务写操作。
...
**押品预警信息**是订单级与押品级风险流水的承接中枢与处置工作台。当前承接由 [01订单预警配置](../../08-配置管理/19-风险管理-订单预警配置/订单预警配置主PRD.md) 与携带有效抵押/质押订单上下文的设备事件触发的 7 类预警；历史监管预警保留为只读档案。当前提供**固定模板风险描述生成、列表展示去重、跳转单据处理（补仓/平仓/部分结清）、超时升级及发起风险公示**的全流程闭环。
...
### 2.1 本期范围
- **预警流水列表与复合检索**：支持按订单号（模糊）、预警类型（当前 7 类，历史监管类型仅归档）、处理状态（未处理/已处理/已失效）、货主名称、货物名称、预警时间范围检索；
...
| **历史解监管超时** | `当前监管物 品类-规格（数量+单位）未在 YYYY年MM月DD日 完成解监管！（预警阈值xxx天）` | 仅用于历史监管预警快照展示，不触发新流水 |
...
| **去处理 (抵质押单据)** | 仅适用于当前抵押/质押订单，必须具备【项目管理/抵质押单据】菜单权限；无权限提示并拦截 | 强校验菜单功能权限、订单类型与订单归属 |
| **解除预警 / 公示风险** | 仅适用于当前抵押/质押订单，必须具备对应按钮操作权限且拥有订单权限 | 服务端二次校验订单类型、记录状态与乐观锁 Version |
...
| **历史监管记录只读** | 历史查询链路 / 旧链接 | 历史监管预警 | 拥有历史记录查看权限 | 否 | 仅查看详情、审计和资料 |
...
1. **订单失效联动**：当前抵押/质押订单货物全部出库或完成对应解押时，关联的未处理押品预警自动更新为 `未处理（无效）`，停止升级通知；历史监管预警不再参与当前处置联动；
...
| 2026-09-11 | V2.0 | 产品团队 | 关闭监管订单当前运营链路；当前新预警、处置和公示仅适用于抵押/质押订单，历史监管预警降为只读兼容数据。 |

...
> **跨文档引用规范**：[跨文档引用口径与来源索引](../../../../B-迭代需求/README.md)。下游引用仓库、库房、分区时使用持久化 ID，并按本模块字段清单和 R31 处理历史快照。
...
| 上游依赖 | **租户隔离标识**：所有仓库、库房、分区及设备设施记录必须带 `tenant_id`；**组织架构人员**：提供仓库监管员、片区经理等人员选择及账号启停状态，来源为 REF-ACCOUNT；**字段基准文档**：本模块字段、枚举、ID 和校验统一以[仓库管理字段清单](./仓库管理字段清单.md)为准。来源：[跨文档引用口径与来源索引](../../../../B-迭代需求/README.md) REF-WH/REF-ACCOUNT。 |

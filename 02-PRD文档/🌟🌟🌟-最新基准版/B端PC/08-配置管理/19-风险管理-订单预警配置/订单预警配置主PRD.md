...
在供应链金融大宗商品存货质押与监管业务中，货权质押期长、市场价格波动大、仓储作业频繁，资金方与监管方需要对质押物进行全生命周期的风险监控；历史监管订单配置仍需满足审计追溯要求。
**订单预警配置**作为订单级风险规则的中枢入口，当前支持针对抵/质押订单配置价格下跌、质押率异常、图像识别、物联网异常、挂锁/门禁、巡检异常、超时解押以及贷中风控模型等 **11 大类预警规则**与动态通知升级策略。历史监管订单规则仅保留只读兼容数据，不参与当前新增、编辑、删除或新预警触发。
...
### 2.1 本期范围
- **规则列表与组合筛选**：支持按规则名称（模糊匹配）、订单号、订单类型（抵/质押、历史监管）、预警类型（多选）、货物名称等多维度组合筛选；
- **订单选择与联动加载**：根据登录账号订单数据权限动态加载当前可运营的抵/质押订单；历史监管订单不进入新增选择器；
...
系统支持 11 类订单预警；当前仅对抵/质押订单开放配置写操作，历史监管订单只读：
...
监管订单历史配置继续保留原订单类型、预警类型、通知对象和版本快照，用于授权查询、详情和审计；不允许通过页面或接口新增、编辑、删除、保存，也不再由历史监管规则触发新一轮预警。
...
| **新增可选订单** | 仅展示当前登录账号有权查看且业务状态处于“抵/质押中”的有效订单；监管订单不返回 | 下拉接口服务端实时校验订单权限、订单生命周期和当前业务开关 |
| **规则详情 / 编辑** | 详情按订单权限开放；监管订单历史规则仅允许详情和审计，不得进入编辑 | 详情与更新接口均在服务端按订单 ID 和业务类型拦截，禁止仅凭规则 ID 越权或绕过关闭开关 |
| **规则删除** | 抵/质押规则按权限执行软删除；监管订单历史规则禁止删除 | 服务端按业务类型阻断监管订单删除请求并记录审计 |
...
| :--- | :--- | :--- | :--- | :---: | :---: |
| **新增规则** | 列表顶部操作栏 | 当前抵/质押订单 | 具备新增权限，存在有效可选抵/质押订单 | 是 | 是，若为贷中风控自动同步台账 |
| **编辑规则** | 列表操作列 | 当前抵/质押规则且生效中 | 具备编辑权限，拥有订单数据权限；历史监管规则只读 | 是 | 是，更新动态阈值与通知对象 |
| **删除规则** | 列表操作列 | 当前抵/质押规则生效中、已失效 | 具备删除权限，拥有订单数据权限；历史监管规则禁止删除 | 是 | 是，关联未处理预警置为无效 |
...
| **ORD-TC02** | 监管订单关闭校验 | 存在历史监管订单或历史监管预警配置 | 新增选择器不返回监管订单；历史监管配置仅可查询、详情和审计；编辑、删除、保存及新预警触发均被阻断 |
...
| **ORD-TC07** | 监管订单历史规则直达旧编辑地址 | 用户持有历史监管规则详情或旧编辑链接 | 页面只读或返回详情；服务端拒绝更新和删除请求，并记录关闭开关审计日志 |
...
| 2026-09-11 | **V1.9** | 产品团队 | 关闭监管订单当前运营链路；新增仅允许抵/质押订单，历史监管配置保留只读查询、审计和兼容快照，补充服务端写入阻断与验收用例。 |

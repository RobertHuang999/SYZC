...
> **模块定位**：供应链金融核心资产抵质押办理、确权开立与风控准入中枢。作为连接资金方（商业银行、保理公司、资方）与货权方（核心企业、借款企业、个体商户）的业务发起中枢，支持针对仓储在库实物【存货】（PC 端主道）以及数字【仓单】（移动端专道）开具规范化质押单与抵押单。通过主体授信资质校验、货值安全线动态监控、货品明细两级级联圈选、SKU 基准价动态试算、多仓单同保管人强一致性约束以及公安级活体人脸识别强核验，构筑“货权清晰、资产可溯、互斥隔离、司法存证”的金融级存货监管防线。  
...
- 在 PC 端为风控与业务人员提供高效的大宗在库【存货】两级级联圈选与授信放款参数录入；
- 在移动端为仓单资产提供具备法律效力的整单质押打包与公安级活体人脸识别强认证；
- 实现从意向发起、押品锁定、风控试算到审批归档的全生命周期强一致性管控。
...
3. **主体信息录入繁琐易错**：货主信用代码、法人证件与联系方式手工录入容易发生笔误，缺乏 OCR 快速识别与企业档案实时带出机制；
...
| :--- | :--- |
| - **双模式主体动态映射**：支持 `企业` 与 `个人` 两种货主类型。企业类型支持名称下拉检索与营业执照拍照 OCR 识别，自动带出统一社会信用代码；个人类型自动带出身份证号与手机号；<br/>- **双单据类型与担保方式自适应**：支持开立 `质押单`（对应 `流动质押` / `静态质押`）与 `抵押单`（对应 `固定抵押` / `浮动抵押`），权人标签与表格表头自适应切换为“质押”或“抵押”；<br/>- **货值安全线风控引擎**：在 `流动质押` 与 `浮动抵押` 模式下自适应唤出货值安全线开关与阈值录入（带单位元/万元切换），固定/静态模式下自动隐蔽；<br/>- **存货两级级联圈选（PC 端主道）**：点击【+ 添加货物】唤出弹窗，支持按仓库、货物多维过滤及时间/数量/生产日期三维排序，以父级（SKU）与子级（具体库位批次）两级树形展现，支持【全部】/【清空】快捷填报与防超额强校验；<br/>- **SKU 基准价动态联动**：货物单价直接继承自货品规格管理（SKU 主档）基准单价，自动计算 `总价 = 数量 × 单价`，无单价时规范展示 `--` 并放行，底部毫秒级汇总品类数、总数量与总价值；<br/>- **仓单质押移动端专道集成**：支持多张数字仓单（CD 编号）复选质押；强校验**整单打包不可拆分**及**多仓单保管人同一性**（黄底红字强提示拦截）；<br/>- **底层货位与仓单互斥锁定**：提交审核后，物理存货状态即刻切为【质押申请中】/【抵押申请中】，仓单状态即刻切为【审批中】，禁止出库、移库、加工与异动；审批通过后分别流转为【质押中】/【抵押中】及【冻结】；<br/>- **移动端活体人脸识别强认证**：以仓单作为标的进行抵质押审批签章时，系统强制调用移动端公安底库活体人脸检测，固化司法不可变存证链。 | - **在 PC 端直接完成仓单人脸认证**：PC 端环境缺乏防作弊合规活体摄像头，仓单抵质押的人脸活体检测强制收拢在移动端（H5/App）执行；<br/>- **拆分单张仓单内部分货物质押**：仓单具有法定不可分证券化属性，严禁单选仓单内某一种或部分数量货物发起抵质押；<br/>- **跨不同保管人合并质押**：严禁将不同保管人（企业或个人）签发的仓单合并在同一笔抵质押单据中提交；<br/>- **抵质押生效后直接手工改单**：终审生效后形成不可变金融质押凭证，严禁手工修改数量、金额或担保物，变更必须走解质押或质押置换流程；<br/>- **本模块直接出具银行批复**：本模块为质押信息录入与申请确权端，信贷审批与放款结果由金融核心系统回写。 |
...
flowchart TD
    subgraph S1[业务准入与模式选择]
        A1[进入抵质押业务办理 P-RZ-004] --> B1[选择货主类型: 企业 / 个人]
        B1 -->|企业| C1[输入公司全称 搜索下拉 / 拍照OCR<br/>自动回显信用代码与电话]
        B1 -->|个人| C2[输入姓名 搜索下拉<br/>自动回显身份证号与手机号]
        C1 & C2 --> D1[选择开立单据类型: 质押单 / 抵押单]
        D1 -->|质押单| E1[选择质押方式: 流动质押 / 静态质押]
        D1 -->|抵押单| E2[选择抵押方式: 浮动抵押 / 固定抵押]
        E1 & E2 --> F1{是否为流动 / 浮动模式?}
        F1 -->|是| G1[展示货值安全线开关与阈值输入]
        F1 -->|否| G2[隐藏货值安全线 仅按批次静态锁定]
    end
    ...
    subgraph S2[标的物圈选与风控试算]
        ...
        I3 --> I4[带入明细表: 获取 SKU 基准价<br/>自动计算总价与底部三项汇总]
        
        H1 -->|数字仓单 移动端专道| J1[移动端抵押/质押单填写<br/>标的物切换为 仓单]
        J1 --> J2[勾选有效电子仓单 CD号<br/>整单全量货物打包锁定]
        J2 --> J3{强校验: 多仓单保管人是否一致?}
        J3 -->|不一致| J4[强拦截弹窗: 保管人冲突禁止提交]
        J3 -->|一致| J5[允许进入下一步]
    end
    ...
    subgraph S3[提审锁定与工作中心流转]
        I4 & J5 --> K1[点击: 提交审核]
        K1 --> L1{标的物类型判断}
        L1 -->|实物存货| M1[物理货位锁定: 质押/抵押申请中<br/>禁止出库/移库/异动/加工]
        L1 -->|数字仓单| M2[仓单状态锁定: 审批中<br/>移动端唤起 法定人脸活体检测 + 协议签署]
        M1 & M2 --> N1[生成 19 位业务单据号<br/>推送至 工作中心-审批中心]
    end

    subgraph S4[审批终审与底账确权]
        N1 --> O1{金融与风控终审}
        O1 -->|审批驳回 / 用户撤回| P1[状态自动回退: 存货恢复 存货中 / 仓单恢复 正常]
        O1 -->|审批通过| Q1[确权生效: 存货置为 质押中/抵押中<br/>仓单置为 冻结<br/>生成生效抵质押台账并可穿透监控]
    end
```
...
│           PC 端业务主道             │           移动端业务专道           │
├────────────────────────────────────┼────────────────────────────────────┤
│ • 办理大宗在库【实物存货】抵质押开立  │ • 办理【数字仓单】整单抵质押开立   │
│ • 支持两级货位级联树形勾选与分配   │ • 多仓单复选与保管人一致性强校验   │
│ • 详细基础信息、授信与安全线配置   │ • 移动端权威公安底库活体人脸识别   │
│ • 查看全息抵质押台账与穿透监控     │ • CA 移动端电子签章合规存证        │
│ • 快捷跳转普通入库发起             │ • 快捷跳转移动端仓单开立           │
└────────────────────────────────────┴────────────────────────────────────┘
```

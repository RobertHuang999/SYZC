...
/** 仅允许新配置和筛选使用启用档，历史流水仍保留原等级快照。 */
export const ENABLED_SEVERITY_LEVELS = SEVERITY_LEVELS
  .filter((level) => level.enabled)
  .sort((a, b) => a.sortOrder - b.sortOrder)

export function getSeverityLevelById(id: string): SeverityLevel | undefined {
  return ENABLED_SEVERITY_LEVELS.find((level) => level.severityLevelId === id)
}

...
export const unlockApprovalConfigListAnnotations: PrototypeAnnotation[] = [
  {
    ...
    title: "开锁审批配置定位与生命周期",
    content:
      "配置管理 → 业务流程管理 → 开锁审批。面向指定门禁设备集中配置开锁授权策略，维护审批节点（本租户全员可选，指定人员含启用/停用）与超时规则，实现货权监管与物防开锁的闭环控制。",
    details: [
      {
        title: "配置生命周期流转图",
        items: [
        ...
        items: [
          {
            label: "本租户全员范围 (P01b)",
            content:
              "审批策略中指定的人员与角色严格限制在当前租户（系统租户）内；指定人员为本租户全部账号（启用/停用均可）；指定角色为本租户启用角色；跨租户数据严禁出现。",
          },
        ],
      },
    ],

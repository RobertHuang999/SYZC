...
export const unlockApprovalConfigFormAnnotations: PrototypeAnnotation[] = [
  {
    ...
    title: "开锁审批配置新增 / 编辑表单",
    content:
      "录入或编辑门禁开锁审批策略，配置适用设备列表、审批节点（指定人员含启用/停用账号，指定角色为启用角色）及审批超时时间。新增保存直接生效，编辑保存生成新版本。",
    details: [
      {
  ...
  },
  {
    ...
    number: 4,
    kind: "规则",
    title: "审批策略：多节点链与超时配置（本租户候选）",
    content:
      "系统默认审批方式为「任一人通过」；支持增删审批节点与拖拽排序；审批对象候选数据为本租户内人员与启用角色（P01b、R02）；审批超时单位内嵌。",
    details: [
      {
        ...
        items: [
          {
            label: "本租户全员范围 (P01b)",
            content:
              "审批对象下拉候选项严格限制在**本租户（系统租户）内**人员账号与系统 RBAC 角色；**指定人员**为本租户内所有账号（**启用/停用均可选择**）；**指定角色**为本租户内**启用**的角色；跨租户人员与角色严禁出现。",
          },
          {
            label: "指定人员展示",
            content:
              "下拉单选展示格式为 **`姓名（所属机构）`**（含启用与停用账号）。",
          },
          {
            label: "指定角色展示",
            content:
              "下拉单选展示格式为 **`角色名称（所属机构）`**（本租户启用角色）。",
          },
        ],
      },
      {
        ...
        items: [
          {
            label: "保存强校验 (R02)",
            content:
              "指定人员须为本租户有效账号（含启用/停用）；指定角色须为本租户启用角色且至少有 1 个成员；否则阻断保存并提示「保存/启用失败：审批节点{序号}无效」。",
          },
          {
            label: "申请时角色展开 (R28/R30)",
            content:
              "开锁申请提交时，系统将指定角色自动展开为当时该角色下所有启用成员账号并固化快照；若展开后 eligible 人员为空（含排除申请人后），由申请侧 R30 阻断提交。",
          },
        ],
      },
      {

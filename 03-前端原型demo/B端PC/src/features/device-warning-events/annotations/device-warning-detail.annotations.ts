    ...
    details: [
      {
        ...
        items: [
          {
            label: "字段清单对照表",
            content: `| 字段名称 / 编码 | 控件与类型 | 展示格式与取值 | 业务规则说明 |
| :--- | :--- | :--- | :--- |
| **事件标识**<br>\`event_id\` | 文本标签 (带复制) | 全局唯一 UUID / 雪花 ID | 系统自动生成；司法审计证据主键，防重防篡改 (DEV-T02) |
| **规则名称快照**<br>\`rule_name\` | 文本只读 | 最大 50 字符；固化触发时刻名称 | 保持触发时刻名称一致；后续规则更名或删除不回写 |
| **预警类型**<br>\`warning_type\` | 枚举徽标 | 5 大类预警大类与细粒子类型 | 明确告警业务归属；通行/安防类禁止自动恢复 |
| **预警等级快照**<br>\`severity_level_id\` | 枚举 Tag 徽章 | 展示等级编码、名称与颜色 | 固化触发时刻租户等级字典；展示 \`sync_to_order_warn\` 穿透标记 |
| **预警时间**<br>\`warning_time\` | 日期时间文本 | \`YYYY-MM-DD HH:mm:ss\` | 物联网网关记录的物理事件发生时间；超时升级起算时刻 |
| **当前状态**<br>\`status\` | 状态 Badge | 待处置·有效(橙) / 已结案·有效(绿) / 已作废(灰) | 严格三态标准词表；已作废展示失效原因与时间 |`,
          },
        ],
      },
    ],

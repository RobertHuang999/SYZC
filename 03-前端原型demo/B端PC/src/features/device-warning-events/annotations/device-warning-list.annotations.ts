...
| **预警内容** | 组合文本 | 位置(仓+库房)+设备名+事实(监测值与安全区间) | 文本截断，悬浮查看完整内容 |
| **预警抓拍图** | 监控联动图片 | 监控抓拍缩略图；私有 OSS 动态时效防盗链 URL | 点击调起高清大图预览大窗 |
| **预警时间** | 事件发生时间 | \`YYYY-MM-DD HH:mm:ss\`；禁止截断年份与秒数 | 列表默认倒序；升级计时起点 |
| **处理信息** | 两行合并展示 | 首行处理人，次行处理时间戳；待处置展示占位横杠“—” | 自动结案展示“系统自动处理” |
| **预警状态** | 状态机枚举 | 三态展示：待处置 · 有效(橙) / 已结案 · 有效(绿) / 已作废(灰) | 严格三态词表，禁止简写 |
...
| :--- | :--- | :--- | :--- |
| **待处置 · 有效** | 人工处置结案 | 【解除预警】、【详情】 | 蓝色高亮；点击弹出单条核销弹窗，需录入 1～200 字情况说明；现场照片选填 |

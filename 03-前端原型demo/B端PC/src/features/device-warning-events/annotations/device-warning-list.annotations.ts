...
| **规则名称**<br>\`rule_name\` | 规则快照文本 | 最大 50 字符；固化触发时刻名称（后续修改不回写） | 点击跳转预警规则详情快照 |
| **预警等级**<br>\`severity_level_id\` | 枚举 Tag 徽标 | 展示等级编码与名称（如 L4 严重风险）；动态色值 | 筛选联动；受穿透开关控制 |
| **预警类型**<br>\`warning_type\` | 两栏树形枚举 | 5 大类枚举与细粒子类型；支持大类/子类精准筛选 | 展开两栏级联多选面板 |
| **预警内容**<br>\`warning_content\` | 组合文本 | 位置(仓+库房)+设备名+事实(监测值与安全区间) | 文本截断，悬浮查看完整内容 |
| **预警抓拍图**<br>\`snapshot_url\` | 监控联动图片 | 监控抓拍缩略图；私有 OSS 动态时效防盗链 URL | 点击调起高清大图预览大窗 |
| **预警时间**<br>\`warning_time\` | 事件发生时间 | \`YYYY-MM-DD HH:mm:ss\`；禁止截断年份与秒数 | 列表默认倒序；升级计时起点 |
| **处理信息**<br>\`handler_info\` | 两行合并展示 | 首行处理人，次行处理时间戳；待处置展示占位横杠“—” | 自动结案展示“系统自动处理” |
| **预警状态**<br>\`status\` | 状态机枚举 | 三态展示：待处置 · 有效(橙) / 已结案 · 有效(绿) / 已作废(灰) | 严格三态词表，禁止简写 |
...
| :--- | :--- | :--- | :--- |
| **待处置 · 有效** | 人工处置结案 | 【解除预警】、【详情】 | 蓝色高亮；点击弹出单条核销弹窗，需录入 >= 10 字情况说明 + 现场照片 |
| **待处置 · 有效** | 恢复自动结案 | 【详情】 (无解除按钮) | 仅由网关遥测平稳 >= 60 秒自动结案，禁止人工干预 |
| **待处置 · 有效** | 触发即结案 | 【详情】 (无解除按钮) | 瞬态通知类，落账即已处理，不提供人工核销按钮 |
| **已结案 · 有效** | 任意策略 | 【详情】 | 查看已结案不可变事实快照、现场核查说明与抓拍图 |
    ...
    G --> I["汇总 Toast 提示：成功 N_succ 条，失败 N_fail 条"]`,
      ...
      },
      {
        ...
        items: [
          {
            label: "核销表单字段规格",
            content: `| 字段名称 / 编码 | 控件与类型 | 必填性 | 校验规则与说明 |
| :--- | :--- | :---: | :--- |
| **情况说明**<br>\`release_remark\` | 多行文本框 | **必填** | 1~200 个字符；禁止全空格；统一写入各选中流水 (DEV-C04) |
| **现场照片**<br>\`evidence_images\` | 图片上传组件 | 选填 | JPG/PNG 格式；单张 $\le$ 5MB，最多 10 张；复制绑定所选流水 |
| **解除预警抓拍图**<br>\`release_snapshot\` | 异步监控抓拍 | 系统自动 | 提交瞬间向同库区摄像头下发抓拍指令；超时容灾不阻断提交 |
| **处理人**<br>\`handler_name\` | 文本只读 | 系统自动 | 格式为“姓名（所属机构名称）”；根据 Token 自动解析固化 |
| **处理时间**<br>\`handle_time\` | 日期时间只读 | 系统自动 | \`YYYY-MM-DD HH:mm:ss\`；事务提交成功的数据库时间戳 |
| **并发版本号**<br>\`version\` | 隐藏字段 | 系统自动 | 乐观锁版本计数器；提交时比对，版本冲突阻断单条提交 (DEV-C03) |`,
          },
        ],
      },
    ],

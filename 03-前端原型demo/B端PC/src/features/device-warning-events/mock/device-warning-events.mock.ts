...
function toRawEvent(event: DeviceWarningEvent): RawDeviceWarningEvent {
  const { dispositionMode: _mode, manualReleaseAllowed: _manual, ...raw } = event
  return raw
}
...
function cloneWithIndex(index: number, template: DeviceWarningEvent): DeviceWarningEvent {
  ...
  const raw: RawDeviceWarningEvent = {
    ...toRawEvent(template),
  ...
  return enrichDeviceWarningEvent(raw)
}

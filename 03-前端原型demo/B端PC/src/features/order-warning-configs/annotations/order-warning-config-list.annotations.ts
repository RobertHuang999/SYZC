...
| **预警订单**<br>\`order_no\` | 订单文本 | 关联真实有效抵押/质押/监管服务订单 | 点击穿透跳转订单详情 (R04 一单一配) |
| **订单类型**<br>\`order_type\` | 枚举 Tag | \`抵押\`、\`质押\`、\`监管服务\`；旧历史监管不展示不映射 | 精确匹配筛选 |
| **已启用预警项**<br>\`active_strategies\` | 标签药丸聚合 | 聚合展示已激活策略 Tag（如【跌价(L3)】【质押率(L2)】） | 抵质押率整卡单等级 (R10) |
| **预警等级分布**<br>\`severity_distribution\` | 徽章色块序列 | 展示各策略命中的 03/01 预警等级颜色序列 | 直观反映订单风险敏感度 |
| **状态**<br>\`status\` | 状态 Badge | 生效中 (绿) / 已失效 (灰，订单办结出库时系统自动置失效) | 联动行操作状态化控制 |

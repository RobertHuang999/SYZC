...
export const deviceWarningConfigFormAnnotations: PrototypeAnnotation[] = [
  ...
  },
  {
    ...
    kind: "规则",
    title: "通知渠道与超时升级表单",
    content: "多选通知渠道；仅勾选短信/邮件后才展示预警对象与升级配置；移动端「设备预警信息」待处置红点自动展示，无需配置；外部通知须配置对象并勾选渠道后才会下发。",
    details: [
      {
        title: "通知与升级约束",
        items: [
          {
            label: "移动端红点与外部通知",
            content: "预警命中时移动端「设备预警信息」入口自动展示待处置红点，无需配置；仅勾选短信/邮件后才展示「预警通知对象」与「启用升级预警」；外部通知须同时配置对象并勾选渠道后才会下发。",
          },
          {
            label: "超时升级天数 (escalation_days)",
            content: "配置升级天数（如 1~30 天）及升级对象；长时间未处置时，仅通过短信向升级对象逐级上报督办（不受首次通知邮件渠道影响）。处置策略为「触发即结案」时不可配置（R15a）。升级计时以每条独立流水的预警时间为起点。",
          },
        ],
      },
    ],

...
import {
  archivedCollateralWarningEventsMock,
  collateralWarningEventsMock,
} from "../mock/collateral-warning-events.mock"
...
export function getCollateralWarningById(
  ...
  const event =
    collateralWarningEventsMock.find((item) => item.eventId === eventId) ||
    archivedCollateralWarningEventsMock.find((item) => item.eventId === eventId)
  if (!event) {
    return null
  }

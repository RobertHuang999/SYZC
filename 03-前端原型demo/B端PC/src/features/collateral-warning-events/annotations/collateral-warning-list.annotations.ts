...
| **预警订单**<br>\`order_no\` | 订单快照文本 | 单行文本；仅抵押、质押、监管服务三类订单 | 点击穿透跳转对应订单详情 |
| **订单类型**<br>\`order_type\` | 枚举 Tag | \`抵押\`、\`质押\`、\`监管服务\`；旧历史监管不展示不映射 | 精确匹配筛选 |
| **预警类型**<br>\`warning_type\` | 枚举 Tag | 7 大类风控类型；设备硬件告警统一归入“物联穿透告警” | 支持多选 OR 组合筛选 |
| **预警等级**<br>\`severity_level_id\` | 枚举 Tag 徽章 | 展示等级编码与名称（如 L4 严重风险）；固化字典快照 | 筛选联动；带穿透标记 |
| **预警来源**<br>\`warn_source\` | 枚举徽标 | \`ORDER_CONFIG\` (订单触发) / \`IOT_PENETRATION\` (物联穿透) | 区分处置路径 (穿透类只读) |
| **预警内容**<br>\`warning_content\` | 模板组装文本 | 参数化模板展示事实与指标差值对比；支持截断悬浮预览 | 贷中展示评分，质押率展示LTV |
| **预警抓拍图**<br>\`snapshot_url\` | 监控联动图片 | 预警触发即时抓拍缩略图；私有 OSS 动态防盗链签名 | 点击调起高清大图预览 |
| **预警时间**<br>\`warning_time\` | 事件发生时间 | \`YYYY-MM-DD HH:mm:ss\`；禁止截断年份与秒数 | 默认倒序；升级起算时刻 |
| **处理信息**<br>\`handler_info\` | 两行合并展示 | 首行处理人，次行处理时间戳；待处置展示占位横杠“—” | 联动解除展示“系统自动处理” |
| **预警状态**<br>\`status\` | 状态机枚举 | 待处置·有效(橙) / 已结案·有效(绿) / 已作废(灰) | 严格三态标准词表 |
| **是否公示**<br>\`is_publicized\` | 公示状态 Tag | \`未公示\` (灰) / \`已公示\` (绿) / \`已取消\` (橙) | 联动公示操作入口 |
...
| :--- | :--- | :--- | :--- | :--- |
| **ORDER_CONFIG** (商业类) | **待处置 · 有效** | 未公示 | 【解除预警】、【详情】 | 点击弹出提示窗展示命中线与可用解除方式，跳转抵质押订单办理补仓/平仓/解押 |
| **IOT_PENETRATION** (物联穿透) | **待处置 · 有效** | 未公示 | 【查看设备事件】、【详情】 | **押品端禁止人工解除**；点击跳转至【设备预警信息 → 详情页】，现场台账解除后自动联动回写 |
| **全部来源** | **已结案 · 有效** | 未公示 | 【公示风险】、【详情】 | 需具备 R-RISK-MGR 权限；点击弹出公示信息录入弹窗，确认后进入风险公示台账 |
| **全部来源** | **已结案 · 有效** | 已公示 / 已取消 | 【查看公示】、【详情】 | 点击直接跳转至【风险公示 → 详情页】查看已发布事实快照与对外披露凭据 |

...
| **严重度排序**<br>\`sort_order\` | 整数数值 | 正整数 (1~20)；数值越大严重度越高 | 决定规则优先级；列表默认按此倒序 |
| **等级编码**<br>\`severity_code\` | 文本输入 | 1~20 字符；大写字母/数字/下划线 (如 \`L1\`, \`P0\`) | 租户内唯一；防重校验 |
| **显示名称**<br>\`severity_name\` | 文本输入 | 1~20 字符；中文描述 (如 \`严重风险\`, \`高危\`) | 告警展示名称；支持编辑 |
| **标签颜色**<br>\`color_hex\` | 颜色选择器 | HEX 色值 (如 \`#EF4444\`)；渲染为彩色药丸 Tag | 告警卡片/列表统一徽标着色 |
| **同步订单预警**<br>\`sync_to_order_warn\` | Switch 开关 | 布尔值；开启展示【是】(蓝Tag)，关闭展示【否】(灰Tag) | **开启时物联穿透生成押品预警** |
| **启用状态**<br>\`is_active\` | 状态 Switch | 开启/停用；停用后下游规则不可选，历史流水不受影响 | 停用后移出下游规则下拉候选 |

...
function resolveLocation(event: CollateralWarningEventDetail): string {
  if (event.penetrationInfo?.triggerLocation) {
    return event.penetrationInfo.triggerLocation
  }

  const snapshot = event.triggerSnapshot ?? ""
  const locationMatch = snapshot.match(/位置[:：]\s*([^；;]+)/)
  return locationMatch?.[1]?.trim() || "仓储监管现场"
}

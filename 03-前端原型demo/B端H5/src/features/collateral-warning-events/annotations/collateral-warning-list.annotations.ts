...
export const collateralWarningListAnnotations: PrototypeAnnotation[] = [
  ...
  },
  {
    ...
    kind: "交互",
    title: "批量风险公示",
    content: "NavBar「批量公示」进入勾选模式；仅【已结案且未公示】卡片可勾选，【已公示】复选框禁用。",
    details: [
      {
        title: "操作规则与权限控制",
        items: [
...
| **预警订单号** | 订单文本 | 抵押/质押/监管服务单号 | 点击跳转详情 (旧监管不展示) |
| **订单类型** | 枚举 Tag | \`抵押\`、\`质押\`、\`监管服务\` | 精确匹配筛选 |
| **预警类型** | 枚举 Tag | 7 大类风控类型；物联穿透统一收敛 | 多选 OR 匹配 |
| **预警等级** | 枚举徽章 | 固化触发时 03/01 等级色块与名称 | 筛选联动；带穿透标记 |
| **预警来源** | 枚举徽标 | \`ORDER_CONFIG\` / \`IOT_PENETRATION\` | 区分处置路径 |
| **预警内容** | 参数化文本 | 依据参数化模板拼接事实与数值差值 | 贷中展示评分，质押率展示LTV |
| **预警抓拍图** | 监控缩略图 | 点击调起大图预览；私有 OSS 签名 | 司法存证快照 |
| **预警状态** | 状态 Badge | 待处置·有效(橙) / 已结案·有效(绿) / 已作废(灰) | 严格三态标准词表 |
| **是否公示** | 公示 Tag | \`未公示\` (灰) / \`已公示\` (绿) / \`已取消\` (橙) | 联动公示入口 |`,
          },
        ],
      },
    ],
...
| :--- | :--- | :--- | :--- | :--- |
| **ORDER_CONFIG** (商业类) | **待处置** | 未公示 | 【解除预警】、【详情】 | 点击跳转融资监管抵质押订单详情办理补仓/平仓/解押 |
| **IOT_PENETRATION** (物联穿透) | **待处置** | 未公示 | 【查看设备事件】、【详情】 | **押品端禁止人工解除**；点击跳转设备预警详情，设备端核销后自动联动回写 |
| **全部来源** | **已结案** | 未公示 | 【公示风险】、【详情】 | 需具备 R-RISK-MGR 权限；点击进入移动端公示确认页 |
| **全部来源** | **已结案** | 已公示 / 已取消 | 【查看公示】、【详情】 | 点击跳转风险公示详情页 |

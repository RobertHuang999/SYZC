...
import { Check, Clock, Copy, KeyRound, UserRound, X } from "lucide-react"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden p-4 space-y-3.5 overscroll-contain">
          {createdApplyNo ? (
            <>
              ...
              <div className="rounded-2xl border border-blue-100 bg-gradient-to-b from-blue-50/90 to-blue-50/30 p-4 text-center">
                ...
                <div className="mt-2 flex items-center justify-center gap-1 text-[11px] text-gray-500">
                  ...
                  <span>有效期至 {validTo.replace("T", " ")} · 可用 {unlockCount} 次</span>
                </div>
              </div>

              {/* R31 规则明确提示 */}
              <div className="rounded-xl bg-blue-50/80 p-2.5 text-xs text-blue-800 border border-blue-200/60 leading-relaxed">
                ℹ️ <strong>R31 安全规范</strong>：人脸门禁不调用短信接口，临时密码仅在当前浮窗及申请详情页展示，请妥善保管。
              </div>
              ...
              <div className="space-y-2 rounded-xl bg-gray-50/80 p-3 text-xs border border-gray-100">
                ...
                <div className="flex justify-between">
                  <span className="text-gray-500">授权开锁次数</span>
                  <span className="font-medium text-gray-800">{unlockCount} 次</span>
                </div>
              </div>
            </>
          ) : (
            <>
              ...
              {/* 规则说明 */}
              <div className="rounded-xl bg-blue-50/70 p-2.5 text-[11px] text-blue-800 border border-blue-200/60 leading-relaxed">
                💡 <strong>R31 说明</strong>：人脸门禁临时密码仅在页面展示，不下发短信；最长有效期 24 小时。
              </div>

...
export function MermaidDiagram({
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-rose-200 bg-rose-50/50 p-3 text-xs text-rose-800">
        ...
        <pre className="mt-1 overflow-x-auto whitespace-pre font-mono text-[11px] text-gray-500">
          {code}
        </pre>
      </div>
    )
  ...
  if (!svgHtml) {
    return (
      <div className="flex h-32 items-center justify-center rounded-lg border bg-gray-50 text-xs text-gray-400">
        正在渲染流程图...
      </div>
    )
  }
  ...
  return (
    <>
      <div
        className={`group relative overflow-hidden rounded-xl border border-gray-200 bg-white p-4 shadow-xs transition-all hover:shadow-md ${className ?? ""}`}
      >
        <div className="absolute top-2 right-2 z-10 opacity-0 transition-opacity group-hover:opacity-100">
          <button
            type="button"
            className="flex items-center gap-1 rounded-md border border-gray-200 bg-white/90 px-2 py-1 text-xs text-gray-600 shadow-xs backdrop-blur hover:bg-gray-100 hover:text-gray-900 cursor-pointer"
      ...
      {isModalOpen && (
        ...
        >
          <div
            className="relative flex h-[min(90vh,48rem)] w-[min(90vw,72rem)] max-w-[90vw] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              <h3 className="text-base font-semibold text-gray-900">流程图全景视图</h3>
              <button
                type="button"

...
| **工作台** | `/m/workspace` | 6大业务分类、5列等宽彩色应用网格（仓储15项/融资监管7项/交易3项/风控4项/结算3项/配置4项；设备管理内含4个 Tab） |
| **业务办理** | `/m/tasks` | 待确认统计、内部审批（待处理/抄送我的/已处理）、业务发起网格、客户需求审批（带红点角标）、其他审批 |
| **机构权限** | `/m/profile` | 登录人信息、授权管辖物理仓库清单、三维权限矩阵规则说明 |
| **我的申请记录** | `/m/my-applies?tab=process\|policy\|unlock-applies` | 6.2 **07/03/04-我的申请管理**；三 Tab（流程申请/政策资讯/开锁审核）对齐 PC |
| **开锁审批（审批人）** | `/m/approval/unlock-applies` | 6.2 **07/05/03-开锁审核**（审批人） |

...
const SCOPE_RULES: Array<{ pattern: RegExp; label: string; kind: DataScopeKind }> = [
  { pattern: /所有机构权限.*所有仓库权限|所有仓库权限.*所有机构权限|全量数据|全部权限/, label: "全局（全域数据）", kind: "all" },
  { pattern: /所有机构权限|所有仓库权限|全部机构|全部仓库/, label: "全局", kind: "all" },
  { pattern: /订单数据权限|可见订单|抵\/质押单|订单相关|交易订单/, label: "订单", kind: "order" },
  { pattern: /仓库权限|仓库 权限|仓库数据|仓库权限内的|所属库区/, label: "仓库", kind: "warehouse" },
  { pattern: /机构权限|机构数据|所属机构|合作机构|数据权限内的机构|顶级机构|本部门/, label: "机构/部门", kind: "org" },
  { pattern: /任务权限|对应任务|任务发起人|任务处理人|待处理|本人经办/, label: "任务/经办", kind: "task" },
  { pattern: /功能权限|仅控制菜单/, label: "功能权限", kind: "function" },
]
...
function detectScopes(text: string): DataScopeTag[] {
  ...
  for (const rule of SCOPE_RULES) {
    if (rule.pattern.test(text)) {
      found.push({ label: rule.label, kind: rule.kind })
    }
  }
